...
def preview_close(session_name: str, total_cash_counted: float) -> dict:
	...
	# Account for mid-shift cash movements (bank drops, change orders, etc.)
	net_cash_movement = _get_net_cash_movement(session_name)
...
# ---------------------------------------------------------------------------
# Cash Movement (mid-shift cash in/out)
# ---------------------------------------------------------------------------


def _get_net_cash_movement(session_name: str) -> float:
	"""Return net cash movement for a session (cash in - cash out)."""
...
def record_cash_movement(
	...
	status = frappe.db.get_value("POS Opening Entry", session_name, "status")
	if status != "Open":
	...
	if movement_type == "Cash Out" and flt(amount) > 100:
		...
		authorized_by = verify_manager_pin(manager_pin)
	...
	direction = "in" if movement_type == "Cash In" else "out"
	return {
		"success": True,
		"movement_name": movement.name,
		"message": _("Cash {0} of ${1} recorded").format(direction, flt(amount)),
	}
...
def get_cash_movements(session_name: str) -> dict:
	...
	movements = frappe.get_all(
		"Cash Movement",
		filters={"session": session_name, "docstatus": 1},
		fields=["name", "movement_type", "amount", "reason", "notes", "authorized_by", "creation"],
