...
def get_revenue_recognition(
	...
	delivered_filters = [["delivered_date", "between", [start_date, end_date]], ["status", "=", "Delivered"]]
	if warehouse:
...
def get_materials_consumed(
	...
	repairs = frappe.get_all("Repair Order", filters=delivered_filters, fields=["name", "material_cost"])
	...
	return {
		...
		"items": sorted(parts_used.values(), key=lambda x: x["total_cost"], reverse=True),
	}
...
def create_journal_entry_for_repair(repair_order: str) -> dict[str, Any]:
	...
	existing_je = frappe.db.get_value(
		"Journal Entry",
		{"reference_name": repair_order, "reference_type": "Repair Order", "docstatus": ["!=", 2]},
	)
	...
	try:
		...
		return {"success": False, "message": f"Failed to create Journal Entry: {e!s}"}
...
def get_profitability_report(
	...
	filters = [["delivered_date", "between", [start_date, end_date]], ["status", "=", "Delivered"]]
	if warehouse:
	...
	repairs = frappe.get_all(
		"Repair Order",
		filters=filters,
		fields=["name", "total_cost", "labor_cost", "material_cost", "warehouse", "repair_type"],
	)
...
def process_commission_payment(
	...
	try:
		...
		pe.paid_to = frappe.db.get_value("Account", {"account_type": "Payable", "company": company})
		pe.payment_type = "Pay"
