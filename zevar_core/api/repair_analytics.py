...
def _build_kpis(wh_cond: str, vals: dict) -> dict[str, Any]:
    ...
    active = cint(frappe.db.sql(  # nosemgrep
        f"""
...
def _sla_metrics(wh_cond: str, vals: dict) -> dict:
    ...
    overdue_now = cint(frappe.db.sql(  # nosemgrep
        f"""
        SELECT COUNT(*) FROM `tabRepair Order`
...
def _customer_insights(wh_cond: str, vals: dict) -> dict:
    ...
    total_customers = cint(frappe.db.sql(  # nosemgrep
        f"""
...
def _build_ai_context(kpis: dict, sla: dict, types: list, techs: list) -> str:
    ...
    for t in techs[:5]:
        lines.append(f"- {t['name']}: {t['completed']}/{t['total']} completed, {t['avg_days']}d avg, ${t['revenue']:,.0f}")
...
def _rule_based_insights(kpis: dict, sla: dict, types: list, techs: list) -> list[str]:
    ...
    if rev_pct > 10:
        ...
        insights.append(f"Repair revenue declined {abs(rev_pct)}% — consider promotions or outreach campaigns.")

    # SLA compliance
    on_time = sla.get("on_time_pct", 0)
    overdue = sla.get("overdue_now", 0)
    if on_time < 80:
        insights.append(f"On-time delivery is only {on_time}%. Review scheduling and parts procurement to improve SLA.")
    elif on_time >= 95:
        insights.append(f"Excellent SLA at {on_time}% on-time delivery — maintain current processes.")

    if overdue > 5:
        insights.append(f"{overdue} repairs are currently overdue. Prioritize these to prevent customer dissatisfaction.")

    # Top repair type insight
    if types:
        top = types[0]
        insights.append(f"'{top['type']}' is the most common repair ({top['count']} orders, {top['pct']}% of volume).")

    # Technician efficiency
    if techs:
        fastest = min(techs, key=lambda t: t["avg_days"]) if techs else None
        if fastest and fastest["avg_days"] > 0:
            insights.append(f"Fastest technician: {fastest['name']} at {fastest['avg_days']} days avg turnaround.")

    # Turnaround warning
    avg_days = kpis.get("avg_turnaround_days", 0)
    if avg_days > 7:
        insights.append(f"Average turnaround is {avg_days} days — consider adding capacity or streamlining QC.")
