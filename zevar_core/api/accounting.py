import frappe
from frappe.utils import cint, cstr, flt, today, now


# ─── ACCOUNTING TRANSACTIONS (Payment Entry + Journal Entry) ──────────────────

@frappe.whitelist(allow_guest=False)
def get_transactions(doctype=None, from_date=None, to_date=None, party=None, account=None, page=1, page_size=20):
    ...
    if not doctype or doctype == "Journal Entry":
        ...
        je_list = frappe.get_all(
            ...
            fields=[
                ...
                "total_credit", "reference_date", "user_remark",
                "docstatus", "cheque_no",
            ],
            order_by="posting_date desc, creation desc",
            limit_start=limit_start if not doctype else 0,
...
@frappe.whitelist(allow_guest=False)
def create_payment_entry(payment_type, party, paid_amount, mode_of_payment,
                         paid_from=None, paid_to=None, reference_no=None, reference_date=None):
...
@frappe.whitelist(allow_guest=False)
def get_invoices(invoice_type="all", status=None, party=None, from_date=None, to_date=None, page=1, page_size=20):
...
def get_invoice_detail(invoice_type, name):
    ...
    return {
        ...
        "invoice": {
            ...
            "customer" if invoice_type == "Sales" else "supplier": doc.customer if invoice_type == "Sales" else doc.supplier,
            "customer_name" if invoice_type == "Sales" else "supplier_name": doc.customer_name if invoice_type == "Sales" else doc.supplier_name,
            "items": items,
            "payments": payments,
        },
    }
...
def get_credit_notes(note_type="all", party=None, from_date=None, to_date=None, page=1, page_size=20):
    ...
    if note_type in ("all", "outgoing"):
        ...
        si_list = frappe.get_all(
            ...
            fields=[
                "name", "customer", "customer_name", "posting_date",
                "grand_total", "return_against", "status", "docstatus",
            ],
            order_by="posting_date desc",
            limit_start=0,
            limit=999,
...
def get_exportable_invoices(invoice_type="sales", from_date=None, to_date=None, page=1, page_size=50):
    ...
    invoices = frappe.get_all(
        ...
        fields=[
            ...
            "customer" if invoice_type == "sales" else "supplier" "as party",
        ],
        order_by="posting_date desc",
        limit_start=limit_start,
        limit=page_size,
...
def export_ubl(invoices_json):
    ...
    for name in invoice_names:
        ...
        export_data.append({
            ...
            "grand_total": flt(doc.grand_total),
            "currency": doc.currency,
            "taxes": [{"account": t.account_head, "rate": flt(t.rate), "amount": flt(t.tax_amount)} for t in doc.taxes],
            "items": [{"item_code": i.item_code, "qty": i.qty, "rate": flt(i.rate), "amount": flt(i.amount)} for i in doc.items],
        })
