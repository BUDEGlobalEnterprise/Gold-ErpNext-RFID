...
def get_calendar_events(
    ...
    repairs = frappe.get_all(
        ...
        filters=filters,
        fields=[
            "name", "status", "priority", "customer", "customer_phone",
            "repair_type", "item_description", "promised_date",
            "received_date", "assigned_to", "warehouse"
        ]
    )
    ...
    for repair in repairs:
        ...
        if "promised" in event_types and repair.get("promised_date"):
            customer_name = frappe.db.get_value("Customer", repair["customer"], "customer_name") if repair.get("customer") else ""
    ...
    if "overdue" in event_types:
        ...
        overdue_repairs = frappe.get_all(
            ...
            fields=[
                "name", "status", "priority", "customer", "customer_phone",
                "repair_type", "promised_date", "warehouse", "assigned_to"
            ]
        )
    ...
    if "pickup" in event_types:
        ...
        pickup_repairs = frappe.get_all(
            ...
            fields=[
                "name", "customer", "customer_phone", "repair_type",
                "promised_date", "warehouse"
            ]
        )
    ...
    if "received" in event_types:
        ...
        received_repairs = frappe.get_all(
            ...
            fields=[
                "name", "customer", "repair_type", "received_date", "warehouse"
            ]
        )
        ...
        for repair in received_repairs:
            ...
            events.append({
                ...
                "start": repair["received_date"].split(" ")[0] if repair.get("received_date") else start_date,
                "allDay": True,
...
def get_technician_schedule(
    ...
    repairs = frappe.get_all(
        ...
        fields=[
            "name", "status", "priority", "customer", "repair_type",
            "item_description", "promised_date", "received_date",
            "labor_cost", "warehouse"
        ]
    )
    ...
    for repair in repairs:
        customer_name = frappe.db.get_value("Customer", repair["customer"], "customer_name") if repair.get("customer") else ""
        repair_type_name = frappe.db.get_value("Repair Type", repair["repair_type"], "repair_name") if repair.get("repair_type") else ""
...
def get_overdue_alerts(
    ...
    overdue_repairs = frappe.get_all(
        ...
        fields=[
            "name", "status", "priority", "customer", "customer_phone",
            "repair_type", "item_description", "promised_date",
            "assigned_to", "warehouse", "total_cost"
        ]
    )
    ...
    for repair in overdue_repairs:
        ...
        if include_customer_info:
            alert["customer"] = frappe.db.get_value("Customer", repair["customer"], "customer_name") if repair.get("customer") else ""
...
def get_ical_feed_url(
    ...
    frappe.cache().set_value(cache_key, {
        ...
        "created": now()
    }, expires_in_sec=365*24*60*60)  # Valid for 1 year
...
def get_upcoming_pickups(
    ...
    pickups = frappe.get_all(
        ...
        fields=[
            "name", "customer", "customer_phone", "repair_type",
            "promised_date", "warehouse", "total_cost", "balance_due"
        ],
    ...
    for pickup in pickups:
        customer_name = frappe.db.get_value("Customer", pickup["customer"], "customer_name") if pickup.get("customer") else ""
...
def get_daily_summary(date: str | None = None, warehouse: str | None = None) -> dict[str, Any]:
    ...
    received_filters = [
        ["received_date", "like", f"{target_date}%"]
    ]
    if warehouse:
        received_filters.append(["warehouse", "=", warehouse])
    ...
    completed_filters = [
        ["completed_date", "like", f"{target_date}%"]
    ]
    if warehouse:
    ...
    delivered_filters = [
        ["delivered_date", "like", f"{target_date}%"]
    ]
    if warehouse:
    ...
    pickups = frappe.get_all(
        "Repair Order",
        filters=pickup_filters,
        fields=["name", "customer", "customer_phone"]
    )
