...
def verify_session(session_token: str, verification_code: str) -> dict[str, Any]:
    ...
    frappe.cache().set_value(auth_cache_key, {
...
@frappe.whitelist(allow_guest=True)
def upload_reference_photo(auth_token: str, repair_order: str, photo_data: str, filename: str | None = None) -> dict[str, Any]:
    """
    ...
    try:
        ...
        doc._log_communication(
            "Customer Portal",
            "Incoming",
            f"Customer uploaded reference photo: {filename}",
            "Customer Portal"
        )
...
@frappe.whitelist(allow_guest=True)
def customer_approve_estimate(auth_token: str, repair_order: str, customer_name: str, notes: str | None = None) -> dict[str, Any]:
    """
...
@frappe.whitelist(allow_guest=True)
def customer_reject_estimate(auth_token: str, repair_order: str, customer_name: str, reason: str) -> dict[str, Any]:
    """
...
def get_repair_history(auth_token: str, include_warranty: bool = True) -> dict[str, Any]:
    ...
    repairs = frappe.get_all(
        ...
        filters={"customer": customer},
        fields=[
            "name", "status", "repair_type", "item_description",
            "received_date", "delivered_date", "total_cost",
            "warranty_months", "warranty_expiry_date",
            "is_warranty_repair", "original_repair_order"
        ],
        order_by="received_date desc"
    )
    ...
    for repair in repairs:
        ...
        if include_warranty:
            warranty_info = {
                ...
                "warranty_expiry_date": str(repair["warranty_expiry_date"]) if repair["warranty_expiry_date"] else None,
            }
...
@frappe.whitelist(allow_guest=True)
def schedule_pickup(auth_token: str, repair_order: str, pickup_date: str, pickup_time: str | None = None, notes: str | None = None) -> dict[str, Any]:
    """
