...
from frappe.query_builder.functions import Coalesce, Count, CurDate, Max, Now, Sum
...
ALL_POS_ROLES = (
	ADMIN_ROLES | MANAGER_ROLES | SALES_ROLES | ACCOUNTING_ROLES | STOCK_ROLES | HR_ROLES | EMPLOYEE_ROLES
)
...
def get_daily_revenue_breakdown(
	...
	date_to: str,
	store: str | None = None,
	_user_roles: set | None = None,
	_role_ctx: dict | None = None,
) -> dict:
	try:
		...
		.select(
			...
			Count(
				frappe.qb.terms.Case().when(
					(lc.target_completion_date >= today_dt) & (lc.target_completion_date <= next_week), 1
				)
			).as_("due_this_week"),
		)
...
@frappe.whitelist(allow_guest=False)
def get_low_stock_detail(
	severity: str = "all", category: str | None = None, limit: int = 50, offset: int = 0
) -> dict:
	...
	query = (
		...
		.join(item_tb)
		.on(bin_tb.item_code == item_tb.name)
		.select(
			bin_tb.item_code,
			item_tb.item_name,
			bin_tb.actual_qty,
			...
			bin_tb.warehouse,
			frappe.qb.terms.Case().when(bin_tb.actual_qty == 0, "stockout").else_("low").as_("severity"),
		)
...
def get_overdue_payments(
	store: str | None = None, type: str = "all", overdue_days_threshold: int = 30
) -> dict:
	repairs = []
	layaways = []
	total_amount = 0.0
...
def _get_overage_candidates(days_threshold: int) -> list[dict]:
	...
	cost_field = (
		item_tb.custom_cost_price if _has_column("Item", "custom_cost_price") else item_tb.valuation_rate
	)
	category_field = (
		item_tb.custom_jewelry_type if _has_column("Item", "custom_jewelry_type") else item_tb.item_group
	)
...
def _compute_overage_score(item: dict) -> int:
	...
	return cint(
		min(
			100,
			max(
				0, round(age_score + velocity_score + margin_score + carrying_score - seasonality_adjustment)
			),
		)
	)
...
def _require_overage_action_role(action_type: str, params: dict) -> None:
	...
	if (
		...
		and markdown_pct > 30
		and not roles.intersection({"System Manager", "Sales Manager", "Store Manager"})
	):
		frappe.throw(_("Manager approval is required for markdowns over 30%."), frappe.PermissionError)
	if not roles.intersection(
		{"System Manager", "Sales Manager", "Store Manager", "Stock Manager", "Inventory Manager"}
	):
...
def _submit_markdown_action(action_type: str, items: list[str], params: dict) -> list[dict]:
	...
	for item_code in items:
		...
		results.append(
			{"item_code": item_code, "pricing_recommendation": doc.name, "clearance_outcome": outcome}
		)
	return results
...
def _submit_note_action(action_type: str, items: list[str], params: dict) -> list[dict]:
	...
	for item_code in items:
		if not frappe.db.exists("Item", item_code):
			frappe.throw(_("Item not found: {0}").format(item_code))
		outcome = _track_clearance_outcome(
			item_code, action_type, flt(params.get("markdown_pct")), None, params
		)
