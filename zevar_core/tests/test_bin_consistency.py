...
class TestBinConsistency(FrappeTestCase):
	...
	def test_no_direct_bin_mutation_in_stock_reservation(self):
		import inspect
		from zevar_core.unified_retail_management_system.doctype.stock_reservation import stock_reservation as sr_module
	...
	def test_bin_matches_serial_count(self):
		warehouses = frappe.get_all(
			"Warehouse",
			filters={"is_group": 0, "company": frappe.defaults.get_global_default("company") or "Zevar Jewelers"},
			pluck="name",
			limit=50,
		)
