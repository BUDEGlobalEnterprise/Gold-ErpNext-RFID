...
def bulk_valuate(gemstone_names: list[str]) -> list[dict]:
    ...
    gems = frappe.get_all(
        ...
        fields=[
            "name", "gemstone_type", "carat_weight", "cost_basis",
            "lab", "cert_number", "color_grade", "clarity_grade",
            "shape", "status",
        ],
    )
...
def _estimate_retail(gem) -> float:
    ...
    Diamond markup: 2.0–3.0× depending on size
    Colored stone markup: 2.5–4.0×
    """
    cost = flt(gem.cost_basis, 2)
    if not cost:
        return 0
