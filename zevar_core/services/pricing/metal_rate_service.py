...
def get_metal_rate(metal_code: str, purity_code: str, rate_source: str = "gold_settings") -> float:
	...
	3. Multiply base rate x fine_metal_content
...
def get_fine_metal_content(purity_code: str, metal: str | None = None) -> float:
	"""
...
def list_metal_rates(active_only: bool = True) -> list[dict]:
	"""
	Return all metal x purity combos with their current rates.
...
def convert_troy_oz_to_grams(troy_oz: float, metal: str | None = None) -> float:
	"""
