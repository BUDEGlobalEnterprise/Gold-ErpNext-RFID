...
def create_task(description: str, assigned_to: str | None = None, priority: str = "Medium", date: str | None = None) -> dict:
    ...
    if target_user != frappe.session.user:
        ...
        if not any(r in roles for r in retail_roles):
            frappe.throw(_("You do not have permission to assign tasks to other users."), frappe.PermissionError)
...
def send_agent_email(recipient: str, subject: str, message: str) -> dict:
    ...
    if not any(r in roles for r in ["System Manager", "Sales User", "POS User"]):
        frappe.throw(_("You do not have permission to send emails through the assistant."), frappe.PermissionError)
...
def get_daily_summary(date: str | None = None) -> dict:
    ...
    try:
        ...
        payment_query = (
            ...
            .join(pos_invoice).on(payment.parent == pos_invoice.name)
            .select(
                payment.mode_of_payment,
                frappe.qb.fn.Sum(payment.amount).as_("amount")
            )
            .where((pos_invoice.posting_date == target_date) & (pos_invoice.docstatus == 1))
