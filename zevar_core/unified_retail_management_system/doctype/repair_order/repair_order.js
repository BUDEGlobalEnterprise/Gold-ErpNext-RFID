...
frappe.ui.form.on("Repair Order", {
	...
	refresh: function (frm) {
		...
		if (frm.doc.stone_weight && frm.doc.stone_weight > 0) {
			frm.dashboard.add_indicator(
				__("Total Stones: {0} ct", [frm.doc.stone_weight.toFixed(2)]),
				"green"
			);
		}
	},
	...
	set_promised_date_from_type: function (frm) {
		...
		frappe.db.get_value("Repair Type", frm.doc.repair_type, "estimated_days").then((r) => {
			if (r.message && r.message.estimated_days) {
				...
				let promised_date = frappe.datetime.add_days(
					received_date,
	...
	get_status_transition_warning: function (frm) {
		...
		if (valid_transitions[previous_status]) {
			if (!valid_transitions[previous_status].includes(current_status)) {
				return __(
					"Warning: Status transition from '{0}' to '{1}' may not be valid. Valid transitions are: {2}",
					[
						previous_status,
						current_status,
						valid_transitions[previous_status].join(", "),
					]
				);
			}
		}
