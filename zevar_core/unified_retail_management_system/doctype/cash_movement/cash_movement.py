...
FINANCIAL_TYPES = ("Cash In", "Cash Out")
POSITIONAL_TYPES = ("Safe Drop", "Bank Drop", "Tender Removal", "Float Entry")
MANAGER_AUTH_TYPES = ("Cash Out", "Float Entry")
...
class CashMovement(frappe.model.document.Document):
	...
	def on_submit(self):
		...
		is_positional = self.movement_type in POSITIONAL_TYPES
		log_event_safely(
	...
	def _validate_manager_authorization(self):
		if self.movement_type == "Cash Out" and flt(self.amount) > 100:
			...
			if (
				...
				and "Store Manager" not in roles
			):
				frappe.throw(_("Authorizing user must be a manager"))

		if self.movement_type == "Float Entry":
			if not self.authorized_by:
				frappe.throw(_("Float entry requires manager authorization"))
			roles = frappe.get_roles(self.authorized_by)
			if (
				"Sales Manager" not in roles
				and "System Manager" not in roles
