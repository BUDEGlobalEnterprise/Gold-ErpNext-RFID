...
zevar.layaway.LayawayPaymentPage = class LayawayPaymentPage {
	...
	render() {
			...
			<div class="zevar-la-shell">
				...
				<div data-region="contract-section" style="display: none;">
					...
					<div class="zevar-la-panel zevar-la-stack-gap">
						...
						<div class="zevar-la-inline-actions">
							<button type="button" class="btn btn-primary" data-action="submit-payment">${__(
								"Process Payment"
							)}</button>
						</div>
					</div>
		...
		this.fields = {
			...
			payment_amount: zevar.layaway.makeControl(
				bodyEl.querySelector('[data-field="payment_amount"]'),
				{
					fieldtype: "Currency",
			...
			mode_of_payment: zevar.layaway.makeControl(
				bodyEl.querySelector('[data-field="mode_of_payment"]'),
				{
					fieldtype: "Link",
			...
			reference_number: zevar.layaway.makeControl(
				bodyEl.querySelector('[data-field="reference_number"]'),
				{
					fieldtype: "Data",
	...
	async selectContract(name) {
		try {
			...
			frappe.show_alert({
				message: __("Contract {0} selected", [name]),
				indicator: "green",
			});
		} catch (error) {
			frappe.msgprint({
	...
	async submitPayment() {
		...
		try {
			...
			const result = await zevar.layaway.call(
				"zevar_core.api.layaway.process_layaway_payment",
				{
			...
			frappe.show_alert({
				message: __("Payment successful. New balance: {0}", [
					zevar.layaway.toCurrency(result.new_balance),
				]),
				indicator: "green",
			});
			this.contractDetail = await zevar.layaway.call(
			...
			if (result.status === "Completed") {
				frappe.msgprint({
					title: __("Layaway Completed!"),
					message: __("The layaway {0} is now fully paid.", [this.selectedContract]),
					indicator: "green",
				});
			}
		} catch (error) {
			frappe.msgprint({
				...
				message: __("Failed to process payment."),
				indicator: "red",
			});
		} finally {
