...
zevar.layaway.CancelLayawayPage = class CancelLayawayPage {
	constructor(wrapper) {
		...
		this.page = frappe.ui.make_app_page({ parent: wrapper, title: __("Cancel Layaway"), single_column: true });
	...
	render() {
			...
			<div class="zevar-la-shell">
				...
				<div data-region="cancel-section" style="display: none;">
					...
					<div class="zevar-la-panel zevar-la-stack-gap zevar-la-warning-panel">
						...
						<div class="zevar-la-inline-actions">
							<button type="button" class="btn btn-danger" data-action="confirm-cancel">${__("Confirm Cancellation")}</button>
						</div>
					</div>
				</div>
			</div>`);
		...
		this.fields = {
			...
			cancellation_reason: zevar.layaway.makeControl(bodyEl.querySelector('[data-field="cancellation_reason"]'),
				{ fieldtype: "Text", fieldname: "cancellation_reason", label: __("Cancellation Reason"), reqd: 1 }),
		};
	...
	async selectContract(name) {
		try {
			...
			if (!["Active", "Overdue"].includes(this.contractDetail.status)) {
				frappe.msgprint(__("Cannot cancel layaway in {0} status.", [this.contractDetail.status])); return;
			}
			...
			frappe.show_alert({ message: __("Contract {0} loaded for cancellation review", [name]), indicator: "orange" });
		} catch (error) {
	...
	confirmCancel() {
		...
		if (!reason || !reason.trim()) { frappe.msgprint(__("Cancellation reason is required.")); return; }
		frappe.confirm(
			__("Are you sure you want to cancel layaway {0}?<br><br>Total paid: <strong>{1}</strong><br><br>This will issue a gift card and cannot be undone.",
	...
	async executeCancellation(reason) {
		...
		try {
			...
			const result = await zevar.layaway.call("zevar_core.api.layaway.cancel_layaway", {
				layaway_id: this.selectedContract, cancellation_reason: reason,
			});
			frappe.show_alert({
				message: __("Layaway cancelled. Gift card {0} issued with {1}.", [result.store_credit_id, zevar.layaway.toCurrency(result.amount_refunded)]),
				indicator: "orange",
			});
			this.selectedContract = null;
			this.contractDetail = null;
			...
			frappe.msgprint({ title: __("Cancellation Failed"), message: __("Failed to cancel layaway."), indicator: "red" });
		} finally {
