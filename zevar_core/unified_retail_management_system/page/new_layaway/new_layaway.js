...
zevar.layaway.NewLayawayPage = class NewLayawayPage {
	...
	render() {
		...
		this.fields = {
			...
			duration: zevar.layaway.makeControl(bodyEl.querySelector('[data-field="duration"]'), {
				fieldtype: "Select", fieldname: "duration", label: __("Duration"), options: ["3", "6", "9", "12"], reqd: 1,
			}, "6"),
			...
			deposit_amount: zevar.layaway.makeControl(bodyEl.querySelector('[data-field="deposit_amount"]'), {
				fieldtype: "Currency", fieldname: "deposit_amount", label: __("Opening Deposit"), reqd: 1,
			}, 0),
			...
			item_code: zevar.layaway.makeControl(bodyEl.querySelector('[data-field="item_code"]'), {
				fieldtype: "Link", fieldname: "item_code", label: __("Item"), options: "Item",
			}),
			qty: zevar.layaway.makeControl(bodyEl.querySelector('[data-field="qty"]'), {
				fieldtype: "Float", fieldname: "qty", label: __("Qty"),
			}, 1),
			rate: zevar.layaway.makeControl(bodyEl.querySelector('[data-field="rate"]'), {
	...
	addItem() {
		...
		if (!itemCode) { frappe.msgprint(__("Select an item before adding it.")); return; }
		if (qty <= 0 || rate <= 0) { frappe.msgprint(__("Quantity and rate must both be greater than zero.")); return; }
	...
	renderItems() {
		...
		if (!this.items.length) {
			$region.html('<div class="zevar-la-empty">Add one or more items to build the contract.</div>');
		} else {
			$region.html(`
				<table class="table table-bordered zevar-la-table">
					...
					<tbody>${this.items.map((item, index) => `
						<tr>
							...
							<td><button type="button" class="btn btn-xs btn-default" data-remove-index="${index}">${__("Remove")}</button></td>
						</tr>`).join("")}
					</tbody>
				</table>`);
		}
		...
		this.$body.find('[data-region="preview"]').html(`
			<div class="zevar-la-summary-grid compact">
				<div class="zevar-la-panel"><div class="zevar-la-eyebrow">Total</div><h3>${zevar.layaway.toCurrency(totals.total)}</h3></div>
				<div class="zevar-la-panel"><div class="zevar-la-eyebrow">Deposit</div><h3>${zevar.layaway.toCurrency(totals.deposit)}</h3></div>
				<div class="zevar-la-panel"><div class="zevar-la-eyebrow">Balance</div><h3>${zevar.layaway.toCurrency(totals.balance)}</h3></div>
				<div class="zevar-la-panel"><div class="zevar-la-eyebrow">Estimated Monthly</div><h3>${zevar.layaway.toCurrency(totals.monthlyEstimate)}</h3></div>
			</div>`);
	}

	async submit() {
		try {
			if (!this.fields.customer.get_value()) { frappe.msgprint(__("Customer is required.")); return; }
			if (!this.items.length) { frappe.msgprint(__("Add at least one item to the contract.")); return; }
			const totals = this.totals;
			if (totals.deposit <= 0 || totals.deposit >= totals.total) {
				frappe.msgprint(__("Deposit must be greater than zero and less than the contract total.")); return;
			}
			...
			frappe.show_alert({ message: __("Layaway {0} created", [result.layaway_id]), indicator: "green" });
			frappe.set_route("Form", "Layaway Contract", result.layaway_id);
		} catch (error) {
			console.error("Failed to create layaway:", error);
			frappe.msgprint({ title: __("Creation Failed"), message: __("Failed to create layaway contract."), indicator: "red" });
		} finally {
			this.$body.find('[data-action="submit"]').prop("disabled", false).text(__("Create Layaway"));
		}
	}
};
