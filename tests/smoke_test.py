...
def test_backend_api_health():
	...
	r = session.get(
		...
		params={"doctype": "Customer", "fields": '["name"]', "limit": 1},
	)
	if r.status_code == 200:
	...
	r = session.post(
		f"{BASE_URL}/api/method/zevar_core.api.customer.quick_create_customer",
		json={"customer_name": test_name, "customer_type": "Individual", "mobile_no": "555-555-9999"},
	)
	if r.status_code == 200:
		data = r.json().get("message", {})
		if data.get("success"):
	...
	r = session.get(
		...
		params={"doctype": "Sales Invoice", "name": "ACC-SINV-2026-00001", "format": "pos_receipt"},
	)
	if r.status_code == 200:
	...
	r = session.post(
		f"{BASE_URL}/api/method/zevar_core.api.pos.calculate_invoice_totals",
		json={
...
def test_frontend_build():
	...
	pf_json = os.path.join(
		...
		"zevar_core",
		"zevar_core",
		"unified_retail_management_system",
		"print_format",
		"pos_receipt",
		"pos_receipt.json",
	)
	...
	cypress_config = os.path.join(BENCH_DIR, "apps", "zevar_core", "tests", "cypress", "cypress.config.js")
