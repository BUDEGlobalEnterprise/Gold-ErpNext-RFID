	...
	<div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
		<div class="max-w-7xl mx-auto p-6 space-y-6">
			...
			<div v-if="loading" class="flex items-center justify-center py-20">
				<div class="animate-spin rounded-full h-8 w-8 border-2 border-gray-300 border-t-emerald-600"></div>
			</div>
					...
					<div class="overflow-x-auto">
						<table class="w-full">
							<thead class="bg-gray-50 dark:bg-slate-700">
								<tr>
									<th v-for="column in currentColumns" :key="column.key" class="table-header">
										{{ column.label }}
									</th>
								</tr>
							</thead>
							<tbody>
								<tr v-if="tableData.length === 0">
									<td :colspan="currentColumns.length" class="text-center py-12 text-gray-500">
										No data found for this period
									</td>
								</tr>
								<tr v-else v-for="row in tableData" :key="row.id" class="table-row">
									<td v-for="column in currentColumns" :key="column.key" class="table-cell">
										<span
...
const currentSummary = computed(() => {
	const s = {
		attendance: [
			{ id: "total", label: "Total Days", value: attendanceSummary.value.total_days || 0, icon: "calendar_month", variant: "primary" },
			{ id: "present", label: "Present", value: attendanceSummary.value.present_days || 0, icon: "check_circle", variant: "success" },
			{ id: "absent", label: "Absent", value: attendanceSummary.value.absent_days || 0, icon: "cancel", variant: "danger" },
			{ id: "rate", label: "Attendance Rate", value: (attendanceSummary.value.rate || "0") + "%", icon: "trending_up", variant: "info" },
		],
		leave: [
			{ id: "total", label: "Total Requests", value: leaveSummary.value.total || 0, icon: "mail", variant: "primary" },
			{ id: "approved", label: "Approved", value: leaveSummary.value.approved || 0, icon: "check_circle", variant: "success" },
			{ id: "pending", label: "Pending", value: leaveSummary.value.pending || 0, icon: "schedule", variant: "warning" },
			{ id: "rejected", label: "Rejected", value: leaveSummary.value.rejected || 0, icon: "cancel", variant: "danger" },
		],
		payroll: [
			{ id: "total", label: "Total Paid", value: "$" + (payrollSummary.value.total_paid || "0.00"), icon: "payments", variant: "success" },
			{ id: "pending", label: "Pending", value: payrollSummary.value.pending_count || 0, icon: "schedule", variant: "warning" },
			{ id: "avg", label: "Avg Net Pay", value: "$" + (payrollSummary.value.avg_net || "0.00"), icon: "account_balance_wallet", variant: "info" },
			{ id: "slips", label: "Total Slips", value: payrollSummary.value.total_slips || 0, icon: "receipt", variant: "primary" },
		],
		expenses: [
			{ id: "total", label: "Total Claimed", value: "$" + (expenseSummary.value.total || "0.00"), icon: "receipt_long", variant: "primary" },
			{ id: "approved", label: "Approved", value: "$" + (expenseSummary.value.approved || "0.00"), icon: "check_circle", variant: "success" },
			{ id: "pending", label: "Pending", value: "$" + (expenseSummary.value.pending || "0.00"), icon: "schedule", variant: "warning" },
			{ id: "rejected", label: "Rejected", value: "$" + (expenseSummary.value.rejected || "0.00"), icon: "cancel", variant: "danger" },
		],
		tasks: [
			{ id: "total", label: "Total Tasks", value: taskSummary.value.total || 0, icon: "assignment", variant: "primary" },
			{ id: "completed", label: "Completed", value: taskSummary.value.completed || 0, icon: "check_circle", variant: "success" },
			{ id: "progress", label: "In Progress", value: taskSummary.value.in_progress || 0, icon: "pending", variant: "warning" },
			{ id: "overdue", label: "Overdue", value: taskSummary.value.overdue || 0, icon: "alarm", variant: "danger" },
		],
	}
...
async function loadPayroll() {
	try {
		...
		payrollSummary.value = { total_paid: "0.00", pending_count: 0, avg_net: "0.00", total_slips: 0 }
		tableData.value = []
	}
}
...
async function loadExpenses() {
	try {
		...
		const records = data || []
		const total = records.reduce((s, r) => s + (parseFloat(r.total_claimed_amount || r.amount || 0)), 0)
		const approved = records.filter((r) => r.status === "Approved" || r.docstatus === 1)
			.reduce((s, r) => s + (parseFloat(r.total_claimed_amount || r.amount || 0)), 0)
		const pending = records.filter((r) => r.status === "Draft" || r.docstatus === 0)
			.reduce((s, r) => s + (parseFloat(r.total_claimed_amount || r.amount || 0)), 0)
		...
		expenseSummary.value = { total: "0.00", approved: "0.00", pending: "0.00", rejected: "0.00" }
		tableData.value = []
	}
}
...
async function loadTasks() {
	try {
		...
		const inProgress = records.filter((r) => r.status === "In Progress" || r.status === "Open").length
		...
		tableData.value = records.map((r) => ({
			...
			status: r.status === "Completed" ? "Completed" : r.status === "Open" ? "Open" : r.status || "Open",
		}))
	} catch {
...
.summary-icon--primary { background: rgba(4, 64, 58, 0.1); color: #04403A; }
.summary-icon--success { background: rgba(16, 185, 129, 0.1); color: #10b981; }
.summary-icon--danger { background: rgba(239, 68, 68, 0.1); color: #ef4444; }
