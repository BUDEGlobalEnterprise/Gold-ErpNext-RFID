	...
	<div class="h-full flex flex-col gap-10 no-scrollbar overflow-y-auto pb-20">
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-8">
			<div class="premium-card !p-8">
				...
				<p class="text-3xl font-black text-gray-900 tracking-tighter leading-none">
					{{ formatCurrency(stats.total_reimbursed) }}
				</p>
			</div>
			...
			<div class="premium-card !p-8">
				<div class="flex items-center justify-between mb-6">
					...
					<span
						class="text-[9px] font-black text-amber-600 bg-amber-50 px-2 py-1 rounded-full"
						>4 items active</span
					>
				</div>
				<p class="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1">
			...
			<div class="premium-card !p-8">
				...
				<div class="flex justify-between">
					<span class="text-[9px] font-bold text-gray-400"
						>{{ formatCurrency(stats.total_spent) }} Spent</span
					>
					<span class="text-[9px] font-bold text-gray-400"
						>{{ formatCurrency(stats.budget_limit) }} Limit</span
					>
				</div>
			</div>
		</div>
		...
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
			...
			<div class="lg:col-span-8 space-y-4">
				...
				>
					<div class="flex items-center gap-6 flex-1">
						...
						<div class="min-w-0">
							<p
								class="text-sm font-black text-gray-900 tracking-tight leading-none mb-1 truncate"
							>
								{{ claim.title || claim.expense_type }}
							</p>
							<div class="flex items-center gap-3">
								<span class="text-[10px] font-bold text-gray-400">{{
									claim.category || "Travel & Logistics"
								}}</span>
			...
			<div class="lg:col-span-4 space-y-10">
				<!-- Distribution -->
				<div class="premium-card !p-8">
					<p class="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6">
						Distribution
					</p>
					<div class="space-y-4">
				...
				<div class="premium-card !p-8 bg-amber-50 border-amber-100">
					<div class="flex items-start gap-4">
						<span class="material-symbols-outlined text-amber-600 text-xl shrink-0"
							>info</span
						>
						<div>
							<p class="text-sm font-black text-gray-900 tracking-tight mb-2">
								Policy Reminder
							</p>
							<p class="text-[11px] font-bold text-gray-500 leading-relaxed">
								All claims over $500 require a digital tax invoice and secondary
								partner approval.
							</p>
						</div>
					</div>
		...
		<Teleport to="body">
			<Transition name="fade">
				...
				>
					...
					></div>
					<div
						class="relative bg-white rounded-4xl p-10 w-full max-w-2xl shadow-2xl border border-gray-50"
					>
						...
						<div class="grid grid-cols-2 gap-8">
							...
							<div>
								...
								<input
									type="number"
									placeholder="0.00"
									class="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 text-gray-900 font-bold text-sm focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all font-mono"
								/>
							</div>
							<div>
							...
							<div class="col-span-2">
								...
								<textarea
									rows="3"
									placeholder="Describe the context of this expense..."
						...
						<div class="flex gap-4 mt-12">
							...
							<button
								class="flex-[1.5] py-4 bg-primary text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-glow-emerald"
							>
								Finalize Submission
							</button>
						</div>
...
const stats = ref({
	total_reimbursed: 14290.5,
	pending_amount: 2840.0,
	total_spent: 13080,
	budget_limit: 20000,
});
