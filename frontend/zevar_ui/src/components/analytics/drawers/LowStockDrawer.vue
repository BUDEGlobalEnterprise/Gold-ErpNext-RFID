		...
		<template #row="{ row }">
			<div class="grid grid-cols-12 gap-2 items-center px-2 py-2 rounded hover:bg-gray-50 dark:hover:bg-[#1C1F26]">
				...
				<div class="col-span-2 text-right">
					<span :class="sevClass(row.severity)" class="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase">{{ row.severity }}</span>
				</div>
			</div>
		</template>
	</DetailListDrawer>
...
const items = ref([])
const columns = [
	{ key: 'item_code' }, { key: 'item_name' }, { key: 'actual_qty' }, { key: 'severity' },
]

onMounted(async () => {
	try {
		const res = await call('zevar_core.api.analytics_hub.get_low_stock_detail', { severity: 'all', limit: 100 })
