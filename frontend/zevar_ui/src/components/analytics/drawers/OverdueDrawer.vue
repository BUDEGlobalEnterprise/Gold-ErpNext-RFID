<template>
	<div class="space-y-4">
		<div>
			<h4 class="text-[10px] font-black uppercase tracking-wider text-gray-500 mb-2">
				Repairs ({{ repairList.length }})
			</h4>
			<DetailListDrawer
...
onMounted(async () => {
	try {
		const res = await call('zevar_core.api.analytics_hub.get_overdue_payments', {
			type: 'all',
		})
