<template>
	<div class="responsive-table-wrapper">
			...
			<table class="w-full text-left">
				...
				<tbody>
					...
					<tr v-if="rows.length === 0">
						<td :colspan="visibleColumns.length + (hasActions ? 1 : 0)" class="px-4 py-12 text-center text-gray-400 dark:text-gray-600">
							<slot name="empty">
		...
		<div v-else class="md:hidden space-y-2">
			...
			>
				...
				<div class="flex items-start justify-between gap-2 mb-2">
					<div class="min-w-0 flex-1">
						<slot :name="`mobile-primary-${primaryColumn?.key || 'name'}`" :row="row" :value="primaryColumn ? row[primaryColumn.key] : ''">
							<div class="text-heading-sm text-gray-900 dark:text-white truncate">
								{{ primaryColumn ? (primaryColumn.format ? primaryColumn.format(row[primaryColumn.key], row) : row[primaryColumn.key]) : '' }}
							</div>
						</slot>
						<div v-if="secondaryColumn" class="text-caption text-gray-500 dark:text-gray-400 mt-0.5 truncate">
							{{ secondaryColumn.format ? secondaryColumn.format(row[secondaryColumn.key], row) : row[secondaryColumn.key] }}
						</div>
					</div>
				...
				<div v-if="mobileFields.length > 0" class="grid grid-cols-2 gap-x-4 gap-y-1">
					<template v-for="col in mobileFields" :key="col.key">
						<div v-if="col.key !== (primaryColumn?.key) && col.key !== (secondaryColumn?.key)">
							<slot :name="`mobile-cell-${col.key}`" :row="row" :value="row[col.key]">
								<span class="text-caption text-gray-400 dark:text-gray-500">{{ col.label }}</span>
			...
			<!-- Empty state -->
			<div v-if="rows.length === 0" class="flex flex-col items-center gap-2 py-12 text-gray-400 dark:text-gray-600">
				<svg class="w-10 h-10 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
				</svg>
				<span class="text-caption">{{ emptyText }}</span>
