	...
	<div class="space-y-4">
		<div class="flex items-center justify-between">
			<h3 class="text-sm font-bold text-gray-900 dark:text-white">
				Margin Heatmap by Category &amp; Metal
			</h3>
		</div>
		...
		<div v-if="rows.length && columns.length" class="premium-card !p-4 overflow-x-auto">
			<table class="w-full text-[10px] border-collapse min-w-[500px]">
				<thead>
					<tr>
						<th
							class="text-left p-2 font-bold text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700"
						>
							Category
						</th>
						<th
				...
				<tbody>
					<tr v-for="row in rows" :key="row">
						<td
							class="p-2 font-medium text-gray-900 dark:text-white border-b border-gray-100 dark:border-gray-800 whitespace-nowrap"
						>
							{{ row }}
						</td>
						<td
						...
						>
							...
							>
								{{
									getCellValue(row, col) !== null
										? getCellValue(row, col).toFixed(1) + '%'
										: '--'
								}}
							</span>
						</td>
		...
		<div v-else class="premium-card !p-8 text-center">
			<p class="text-xs text-gray-400">
				No heatmap data available. Ensure items have category and metal type assignments.
			</p>
		</div>

		<!-- Legend -->
		<div class="premium-card !p-4">
			<p class="text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-2">
				Margin Ranges
			</p>
...
function getCellValue(rowLabel, metalType) {
	...
	const row = data.find((r) => (r.jewelry_type || r.category || '') === rowLabel)
