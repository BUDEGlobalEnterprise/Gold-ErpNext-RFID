<template>
	<BaseModal :show="true" max-width="max-w-md" @close="$emit('close')">
		...
		<div class="p-6 space-y-4">
			<div class="bg-gray-50 dark:bg-warm-dark-900 rounded-lg p-4 mb-4">
				...
				<div class="flex justify-between mb-2">
					...
					<span class="font-medium text-green-600"
						>${{ formatNum(previousPayments) }}</span
					>
				</div>
			...
			<form @submit.prevent="submitPayment" class="space-y-4">
				<div>
					...
					<div class="relative">
						...
						<input
							...
							required
							placeholder="0.00"
							class="w-full pl-7 pr-4 py-2 border rounded-lg bg-white dark:bg-warm-dark-900"
						/>
					</div>
					<button
						type="button"
						@click="form.amount = remainingBalance"
						class="mt-1 text-xs text-[#D4AF37] hover:underline"
					>
				...
				<div class="flex gap-3">
					...
					<button
						type="submit"
						:disabled="submitting"
						class="flex-1 py-2 bg-green-500 text-white rounded-lg text-sm font-medium hover:bg-green-600 disabled:opacity-50"
					>
