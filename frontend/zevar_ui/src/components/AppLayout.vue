...
const isEmployeeOnly = computed(() => {
	return session.hasAnyRole(['Employee', 'Employee Self Service', 'Sales Associate']) && !session.isAdmin
})
...
const showFilterSidebar = computed(() => {
	...
	return ['/terminal', '/inventory', '/pos-catalogue'].includes(path) || path.startsWith('/catalogues')
})
