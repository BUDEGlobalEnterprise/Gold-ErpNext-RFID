	...
	<div class="repair-photo-gallery">
		...
		<div class="flex items-center justify-between mb-3">
			<h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider">
				Photo Documentation
				<span v-if="allPhotos.length" class="ml-1 text-[10px] font-normal text-gray-400"
		...
		<div v-if="!showComparison">
			...
			<div v-if="beforePhotos.length" class="mb-3">
				<p
					class="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase mb-1.5 flex items-center gap-1"
				>
			...
			<div v-if="afterPhotos.length">
				<p
					class="text-[10px] font-bold text-green-600 dark:text-green-400 uppercase mb-1.5 flex items-center gap-1"
				>
		...
		>
			...
			<div class="absolute top-4 left-4 z-10">
				...
				>
					{{ lightboxType === 'before' ? 'BEFORE' : 'AFTER' }} {{ lightboxIdx + 1 }}/{{
						lightboxType === 'before' ? beforePhotos.length : afterPhotos.length
					}}
				</span>
			</div>
