	...
	<div class="space-y-4">
		...
		<div class="flex gap-2 overflow-x-auto pb-2">
			<button
				...
				class="h-8 px-3 rounded-full border text-[11px] font-bold whitespace-nowrap flex items-center gap-1.5"
				:class="
		...
		<div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 pb-4">
			<div
				...
				:key="report.id"
				class="premium-card !rounded-xl !p-4 text-left hover:!border-[#D4AF37] focus:outline-none focus:ring-2 focus:ring-[#D4AF37] transition-all group relative"
			>
				<button
				...
				>
					...
					>
						<span
							...
							>{{ groupIcon(report.group) }}</span
						>
					</div>
					<div class="min-w-0 flex-1">
						<div
							class="flex items-start justify-between gap-2"
							:class="{ 'pr-8': canSubscribe }"
						>
							...
							<span
								...
								class="shrink-0 px-1.5 py-0.5 rounded-full bg-[#D4AF37]/15 text-[#9A6B00] dark:text-[#F2D675] text-[9px] font-bold uppercase"
								>Core</span
							>
						</div>
						<p
							class="text-[11px] leading-4 text-gray-500 dark:text-gray-400 mt-1 line-clamp-2"
						>
							{{ report.description }}
						</p>
						<div class="flex items-center gap-2 mt-2">
							<span
								...
								>{{ formatSensitivity(report.sensitivity) }}</span
							>
							...
							<span
								class="ml-auto material-symbols-outlined !text-sm text-gray-400 group-hover:text-[#D4AF37] transition-colors"
								>open_in_new</span
							>
						</div>
					</div>
				...
				<button
					v-if="canSubscribe"
					@click.stop="$emit('subscribe', report.id)"
					class="absolute top-3 right-3 w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-[#D4AF37] hover:bg-[#D4AF37]/10 transition-all"
		...
		<div
			...
			class="p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50"
		>
			<div class="flex items-start gap-2">
				<span class="material-symbols-outlined !text-base text-blue-500 mt-0.5">info</span>
				<p class="text-xs text-blue-700 dark:text-blue-300">
					You are viewing reports scoped to <strong>your own sales</strong>.
				</p>
			</div>
		</div>
	</div>
...
const canSubscribe = computed(() => {
	return ['System Manager', 'Store Manager', 'Sales Manager', 'Accounts Manager'].some((r) =>
		[props.roleContext.primary_role].includes(r)
	)
})
...
function sensitivityClass(s) {
	return (
		...
		}[s] || 'text-green-600 dark:text-green-300'
	)
}
