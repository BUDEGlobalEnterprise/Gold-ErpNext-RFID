<template>
	<div
		class="technician-workload-panel bg-white dark:bg-warm-dark-800 rounded-xl border border-gray-100 dark:border-warm-border p-4"
	>
		...
		<div v-if="loading && !technicians.length" class="text-center py-6">
			<div
				class="animate-spin w-6 h-6 border-2 border-[#D4AF37] border-t-transparent rounded-full mx-auto"
			></div>
			...
			>
				...
				<div class="flex items-center justify-between mb-2">
					<div class="flex items-center gap-2">
						<div
							class="w-7 h-7 rounded-full bg-gradient-to-br from-[#D4AF37] to-[#B8962A] flex items-center justify-center text-[10px] font-bold text-white"
						>
							{{ getInitials(tech.user_name) }}
						</div>
						<div>
							<p
								class="text-xs font-bold text-gray-900 dark:text-white leading-tight"
							>
								{{ tech.user_name }}
							</p>
							<p class="text-[9px] text-gray-400">
								{{ tech.avg_turnaround_days }}d avg turnaround
							</p>
						</div>
					</div>
				...
				<!-- Workload Bar -->
				<div
					class="relative h-3 bg-gray-100 dark:bg-warm-dark-900 rounded-full overflow-hidden mb-2"
				>
					<div
				...
				<!-- Stats Row -->
				<div
					class="flex items-center justify-between text-[9px] text-gray-400 pt-1 border-t border-gray-50 dark:border-warm-border/20"
				>
					<span>{{ tech.completed_this_month }} completed this month</span>
					<span class="font-bold text-green-600"
						>${{ formatNum(tech.revenue_this_month) }}</span
					>
				</div>
			</div>
		</div>
		...
		>
			...
			<p class="text-xs text-blue-700 dark:text-blue-300">
				...
				<span class="text-[9px] opacity-70">
					({{ suggestion.active_jobs }} active, {{ suggestion.avg_days }}d avg,
					{{ suggestion.type_experience }} similar completed)
				</span>
			</p>
		</div>
	</div>
