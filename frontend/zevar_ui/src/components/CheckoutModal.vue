<template>
	...
	>
		...
		<template v-if="step === 'review'">
			<div class="flex flex-col md:flex-row" style="height: 80vh; min-height: 500px; max-height: 820px;">
				<!-- LEFT COLUMN — context-sensitive -->
				<div class="w-full md:w-[45%] bg-gray-50 dark:bg-[#15171e] p-6 border-r border-gray-100 dark:border-warm-border/50 flex flex-col overflow-y-auto">
					...
					<template v-if="mode === 'sale'">
						<div class="flex-1 overflow-y-auto space-y-2 pr-1 mb-4">
							<div v-for="item in cart.items" :key="item.item_code"
								...
								<div class="flex items-center gap-3 min-w-0">
									<div class="w-10 h-10 bg-gray-100 dark:bg-warm-dark-900 rounded-md overflow-hidden flex-shrink-0">
										<img v-if="item.image" :src="item.image" class="w-full h-full object-cover" />
									</div>
									<div class="min-w-0">
										<div class="font-bold text-gray-900 dark:text-white text-sm truncate">{{ item.item_name }}</div>
						...
						<div class="pt-3 border-t border-gray-200 dark:border-warm-border">
							<label class="flex items-center justify-between cursor-pointer group">
								<div>
									<span class="font-medium text-gray-700 dark:text-gray-300 text-sm">Tax Exempt</span>
									<span class="text-xs text-gray-400 block">For resellers or tax-free sales</span>
								</div>
								<button @click="taxExempt = !taxExempt"
									class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors"
									:class="taxExempt ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'">
									<span class="inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform"
										:class="taxExempt ? 'translate-x-6' : 'translate-x-1'"></span>
								</button>
							</label>
						</div>

						<!-- Totals -->
						<div class="mt-4 space-y-2 pt-4 border-t border-gray-200 dark:border-warm-border">
							...
							<div v-if="tradeInTotal > 0" class="flex justify-between text-sm font-bold text-orange-600 dark:text-orange-400">
								<span>Trade-In Credit</span><span>-{{ formatCurrency(tradeInTotal) }}</span>
							</div>
							<div class="flex justify-between text-2xl font-bold text-gray-900 dark:text-white pt-2 border-t border-gray-200 dark:border-warm-border mt-2">
					...
					<template v-else>
						<div class="bg-gradient-to-br from-orange-50 to-orange-100/50 dark:from-orange-900/20 dark:to-orange-800/10 rounded-xl p-5 border border-orange-200 dark:border-orange-800/30 mb-5">
							<div class="flex items-center justify-between">
								<span class="text-sm text-orange-700 dark:text-orange-300 font-medium">Outstanding Balance</span>
								<span class="text-2xl font-bold text-orange-700 dark:text-orange-300">{{ formatCurrency(totalAmount) }}</span>
							</div>
							<p v-if="referenceId" class="text-xs text-orange-600/70 dark:text-orange-400/70 mt-2">
								{{ mode === 'layaway' ? 'Layaway' : 'Repair' }}: {{ referenceId }}
							</p>
						</div>

						<div v-if="selectedPayments.length > 0" class="flex-1 overflow-y-auto pr-1">
							<h4 class="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Payment Breakdown</h4>
							<div class="space-y-2">
								<div v-for="payment in selectedPayments" :key="payment.mode"
									...
									<div class="flex items-center gap-2">
										...
										<input type="number" v-model.number="payment.amount" @input="recalculateSplit(payment.mode)"
							...
							<div class="flex justify-between text-sm pt-3 mt-3 border-t border-gray-200 dark:border-warm-border">
								...
								<span :class="remainingAmount === 0 ? 'text-green-500 font-bold' : remainingAmount < 0 ? 'text-orange-500 font-bold' : 'text-red-500 font-bold'">
					...
					<!-- Error -->
					<div v-if="error" class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/30 rounded-xl p-3 mt-3">
						<p class="text-sm text-red-600 dark:text-red-400">{{ error }}</p>
					</div>
				</div>

				<!-- RIGHT COLUMN — Payment Methods (shared) -->
				<div class="w-full md:w-[55%] p-6 flex flex-col bg-white dark:bg-[#1a1c23] overflow-y-auto">
					<div class="flex items-center justify-between mb-1">
						<h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ rightPanelHeading }}</h2>
					</div>
					<p class="text-sm text-gray-500 dark:text-gray-400 mb-5">{{ rightPanelSubtext }}</p>

					<!-- SALE MODE: Sales Associates -->
					<div v-if="mode === 'sale'" class="mb-4">
						<button @click="showSalespersons = !showSalespersons"
							class="w-full flex items-center justify-between p-3 border rounded-xl transition-all"
							:class="cart.salespersons.length > 0 ? 'border-[#D4AF37] bg-[#D4AF37]/10' : 'border-gray-200 hover:border-gray-400 dark:border-warm-border dark:hover:border-white/30'">
							<div class="flex items-center gap-3">
								...
								<div class="text-left">
									<span class="font-medium text-gray-900 dark:text-white text-sm">Sales Associates</span>
									<span v-if="cart.salespersons.length > 0" class="text-xs text-gray-500 dark:text-gray-400 block">{{ cart.salespersons.length }} assigned</span>
								</div>
							</div>
							<svg class="w-4 h-4 text-gray-400 transition-transform" :class="showSalespersons ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
						</button>
						<div v-if="showSalespersons" class="mt-2 bg-gray-50 dark:bg-warm-dark-700 rounded-xl p-4 border border-gray-100 dark:border-warm-border/50 space-y-3">
							<div v-for="(sp, idx) in cart.salespersons" :key="idx" class="flex items-center gap-2">
								<select v-model="sp.employee" class="flex-1 px-3 py-2 bg-white dark:bg-warm-dark-700 border border-gray-200 dark:border-warm-border rounded-lg text-sm">
									<option value="" disabled>Select Associate</option>
									<option v-for="emp in salesAssociates.data" :key="emp.name" :value="emp.name">{{ emp.employee_name }} ({{ emp.designation || 'Sales' }})</option>
								</select>
								<div class="flex items-center gap-1">
									<input v-model.number="sp.split" @input="cart.recalculateSalespersonSplit(idx)" type="number" min="0" max="100"
								...
								<button @click="cart.removeSalesperson(idx)" class="p-1 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors">
									<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
								</button>
							</div>
							<button v-if="cart.salespersons.length < 4" @click="cart.addSalesperson('', null)"
								class="w-full py-2 text-sm font-medium text-[#D4AF37] border border-dashed border-[#D4AF37]/40 rounded-lg hover:bg-[#D4AF37]/10 transition">+ Add Associate</button>
							<div v-if="cart.salespersons.length > 0 && salespersonSplitTotal !== 100" class="text-xs text-red-500 font-medium">
					...
					<div v-if="mode === 'sale' && isPaymentSelected('Gift Card')"
						class="bg-gray-50 dark:bg-warm-dark-700 rounded-xl p-4 mb-4 border border-gray-100 dark:border-warm-border/50">
						<label class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block">Gift Card Number</label>
						<div class="flex items-center gap-2">
							<input v-model="giftCardNumber" type="text" placeholder="Enter card number" @blur="lookupGiftCard" @keyup.enter="lookupGiftCard"
							...
							<button @click="lookupGiftCard" :disabled="!giftCardNumber || giftCardLoading"
						...
						<div v-if="giftCardInfo" class="mt-2 text-sm" :class="giftCardInfo.valid ? 'text-green-600' : 'text-red-500'">
							<span v-if="giftCardInfo.valid">Balance: {{ formatCurrency(giftCardInfo.balance) }}</span>
							<span v-else>{{ giftCardInfo.message }}</span>
						</div>
					</div>

					<!-- Split Amounts (sale mode, multiple payments) -->
					<div v-if="mode === 'sale' && selectedPayments.length > 1"
						class="bg-gray-50 dark:bg-warm-dark-700 rounded-xl p-4 mb-4 border border-gray-100 dark:border-warm-border/50">
						...
						<div class="flex justify-between text-sm pt-2 border-t border-gray-200 dark:border-warm-border mt-2">
							<span class="text-gray-500">{{ remainingAmount < 0 ? 'Change Due' : 'Remaining' }}</span>
							<span :class="remainingAmount === 0 ? 'text-green-500 font-bold' : remainingAmount < 0 ? 'text-orange-500 font-bold' : 'text-red-500 font-bold'">
								{{ remainingAmount < 0 ? formatCurrency(Math.abs(remainingAmount)) : formatCurrency(remainingAmount) }}
							</span>
					...
					<div class="flex-shrink-0">
						<button @click="handlePayment" :disabled="!canSubmit || processing"
							...
							:class="!canSubmit || processing ? 'bg-gray-100 text-gray-400 cursor-not-allowed dark:bg-warm-dark-700 dark:text-gray-600' : 'bg-gray-900 text-white hover:bg-black dark:bg-[#D4AF37] dark:text-black dark:hover:bg-[#b5952f]'">
		...
		<template v-if="step === 'success'">
			<div class="p-12 flex flex-col items-center justify-center text-center w-full" style="min-height: 400px;">
				...
				<h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">{{ successTitle }}</h2>
				<p class="text-gray-500 dark:text-gray-400 mb-6">{{ successSubtext }}</p>

				<div class="bg-gray-50 dark:bg-[#15171e] rounded-xl p-5 w-full max-w-sm mb-6 border border-gray-100 dark:border-warm-border/50 space-y-2">
					<div v-for="(p, idx) in successBreakdown" :key="idx"
						class="flex justify-between text-sm py-2" :class="{ 'border-t border-gray-200 dark:border-warm-border': idx > 0 }">
					...
					<div class="flex justify-between text-sm pt-3 mt-2 border-t border-gray-200 dark:border-warm-border">
						<span class="font-bold text-gray-900 dark:text-white">{{ mode === 'sale' ? 'Total Paid' : 'Total Paid' }}</span>
						<span class="font-bold text-[#D4AF37]">{{ formatCurrency(successBreakdown.reduce((s, p) => s + p.amount, 0)) }}</span>
					</div>
					<div v-if="successInvoiceId" class="flex justify-between text-sm pt-3 mt-2 border-t border-gray-200 dark:border-warm-border">
						<span class="text-gray-500 dark:text-gray-400">Transaction ID</span>
						<span class="font-mono font-bold text-gray-900 dark:text-white">{{ successInvoiceId }}</span>
					</div>
				</div>

				<div class="flex gap-3 w-full max-w-sm">
					<button @click="close" class="flex-1 py-3 rounded-xl font-bold bg-gray-900 dark:bg-[#D4AF37] text-white dark:text-black hover:bg-gray-800 dark:hover:bg-[#b5952f] transition">
						{{ mode === 'sale' ? 'New Order' : 'Done' }}
					</button>
					<button v-if="mode === 'sale'" @click="printReceipt"
...
const props = defineProps({
	...
	mode: { type: String, default: 'sale', validator: v => ['sale', 'layaway', 'repair'].includes(v) },
...
const salespersonSplitTotal = computed(() => cart.salespersons.reduce((sum, sp) => sum + (sp.split || 0), 0))
const tradeInTotal = computed(() => cart.tradeIns.reduce((sum, ti) => sum + (ti.trade_in_value || 0), 0))
...
const canSubmit = computed(() => {
	...
	if (props.mode === 'sale' && cart.salespersons.length > 0 && Math.abs(salespersonSplitTotal.value - 100) > 0.01) return false
...
function recalculateSplit(changedMode) {
	...
	if (props.mode === 'sale' && changedMode === 'Gift Card' && giftCardInfo.value?.valid) {
		if (changed && changed.amount > giftCardInfo.value.balance) changed.amount = Number(giftCardInfo.value.balance.toFixed(2))
	}
...
async function lookupGiftCard() {
	...
	try {
		const r = await createResource({ url: 'zevar_core.api.gift_card.get_gift_card_balance', params: { gift_card_number: giftCardNumber.value } }).fetch()
		const data = r?.data || r
...
async function handlePayment() {
	...
	if (props.mode === 'sale') {
		const gc = selectedPayments.value.find(p => p.mode === 'Gift Card')
		if (gc && (!giftCardInfo.value?.valid || !giftCardNumber.value)) { error.value = 'Please enter and verify a valid Gift Card number.'; return }
		if (gc && gc.amount > (giftCardInfo.value?.balance || 0)) { error.value = 'Gift Card amount exceeds available balance.'; return }
	}
	...
	try {
		const payments = selectedPayments.value.filter(sp => sp.amount > 0).map(sp => ({ mode_of_payment: sp.mode, amount: sp.amount }))
		...
		if (props.mode === 'sale') {
			...
			const splitPaymentResource = createResource({ url: 'zevar_core.api.layaway.process_split_layaway_payment', auto: false })
			const rawResult = await splitPaymentResource.submit({ layaway_id: props.referenceId, payments: JSON.stringify(payments) })
		...
		if (e?.exc_type === 'ValidationError' && e?.message) {
			...
			try { const msgs = JSON.parse(e._server_messages); errorMsg = msgs.map(m => { try { return JSON.parse(m).message } catch { return m } }).join('\n') } catch { errorMsg = String(e._server_messages) }
		} else {
