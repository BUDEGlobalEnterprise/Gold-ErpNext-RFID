<template>
	<BaseModal :show="true" max-width="max-w-2xl" scrollable @close="$emit('close')">
			...
			<div class="flex items-center gap-3">
				...
				<div>
					<h3 class="text-lg font-bold text-gray-900 dark:text-white">
						{{ mode === 'out' ? 'Consignment Out' : 'Consignment Return' }}
					</h3>
				</div>
			</div>
		</template>
			...
			<div v-if="mode === 'out'" class="space-y-3">
				<div>
					...
					<input
						...
						placeholder="e.g., Bridal Show 2026"
					/>
				</div>
				<div>
		...
		<template #footer>
			...
			<button
				@click="submit"
				:disabled="submitting || serialNos.length === 0 || !eventName"
				class="flex-1 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 disabled:opacity-50"
			>
				{{
					submitting
						? 'Processing...'
						: mode === 'out'
						? `Consign ${serialNos.length} Pieces`
						: `Return ${serialNos.length} Pieces`
				}}
			</button>
		</template>
	</BaseModal>
...
async function submit() {
	...
	try {
		...
		const payload =
			props.mode === 'out'
				? {
						items: serialNos.value.map((sn) => ({ serial_no: sn })),
