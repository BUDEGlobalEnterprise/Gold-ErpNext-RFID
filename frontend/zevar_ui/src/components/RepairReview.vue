<template>
	<div class="mb-6 p-4 bg-white dark:bg-warm-dark-900 rounded-lg border border-gray-200 dark:border-warm-border">
		...
		<div v-if="submitted" class="text-center p-6 bg-green-50 dark:bg-green-900/20 rounded-lg">
			<div class="w-12 h-12 bg-green-100 dark:bg-green-800 rounded-full flex items-center justify-center mx-auto mb-3 text-green-600 dark:text-green-400">
				<span class="material-symbols-outlined !text-2xl">thumb_up</span>
			</div>
			<p class="font-bold text-green-700 dark:text-green-300">Thank you for your feedback!</p>
			<p class="text-sm text-green-600 dark:text-green-400 mt-1">We appreciate you choosing Zevar Jewelers.</p>
		</div>

		<form v-else @submit.prevent="submitReview" class="space-y-4">
			<div class="flex flex-col items-center py-4">
				<p class="text-sm text-gray-600 dark:text-gray-400 mb-3">How would you rate your repair?</p>
				<div class="flex gap-2">
					...
					>
						<span class="material-symbols-outlined !text-3xl transition-colors"
							:class="(hoverRating ? star <= hoverRating : star <= rating) ? 'text-[#D4AF37]' : 'text-gray-300 dark:text-gray-600'">
							star
						</span>
					</button>
				</div>
			...
			<div>
				<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Any comments or feedback?</label>
