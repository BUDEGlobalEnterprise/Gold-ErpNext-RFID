<template>
	<BaseModal :show="true" max-width="max-w-4xl" @close="$emit('close')">
				...
				<div class="flex items-center gap-3">
					...
					<div>
						...
						<p class="text-xs text-gray-500 dark:text-gray-400">
							Create a secure, comprehensive jewelry repair order
						</p>
					</div>
				</div>
				<button
		...
		<!-- Stepper Tabs -->
		<div
			class="border-b border-gray-100 dark:border-warm-border/50 bg-gray-50/50 dark:bg-warm-dark-900/30 px-6 py-3 flex-shrink-0 flex items-center justify-between"
		>
					...
					<span
						...
						:class="
							activeTab === tab.id
								...
								: 'bg-gray-200 dark:bg-warm-dark-800 text-gray-500'
						"
					>
						{{ tab.num }}
					</span>
		...
		<div class="p-6 overflow-y-auto max-h-[60vh] custom-scrollbar">
			<div
				v-if="errorMsg"
				class="mb-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/30 rounded-lg text-sm text-red-700 dark:text-red-400"
			>
				{{ errorMsg }}
			</div>
			...
			<div v-if="activeTab === 'logistics'" class="space-y-6">
				<!-- Customer Section -->
				<div
					class="bg-gray-50 dark:bg-warm-dark-900/50 rounded-xl p-5 border border-gray-100 dark:border-warm-border/30"
				>
					...
					<div class="relative">
						...
						/>
						<svg
							class="w-4 h-4 text-gray-400 absolute left-3 top-3.5"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
						...
						>
							...
							>
								<div>
									...
									<span
										v-if="c.is_repeat"
										class="ml-2 text-[10px] bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full font-bold"
										>✨ Repeat Client</span
									>
								</div>
					...
					<div
						v-if="form.customer"
						class="mt-3 p-3 bg-white dark:bg-warm-dark-900/80 rounded-lg border border-gray-100 dark:border-gray-800/80 flex items-center justify-between"
					>
						<div class="flex items-center gap-2">
							...
							<span
								...
								class="text-xs text-amber-500 font-bold bg-amber-50 dark:bg-amber-900/10 px-2 py-0.5 rounded"
							>
					...
					<div
						...
						class="mt-3 p-3 bg-white dark:bg-warm-dark-900 rounded-lg border border-gray-100 dark:border-gray-800 text-xs"
					>
						<p
							class="font-bold text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-1.5"
						>
							<svg
						...
						<div
							class="space-y-3 relative border-l border-gray-100 dark:border-gray-800 ml-2 pl-4"
						>
							<div v-for="h in customerRepairHistory" :key="h.name" class="relative">
								<div
									class="absolute -left-[21px] top-1.5 w-2.5 h-2.5 rounded-full bg-gray-300 dark:bg-gray-700"
								></div>
								<div class="flex justify-between items-start">
									<div>
										...
										<p class="text-gray-400 mt-0.5 text-[10px]">
											{{ h.item_description || 'No description' }}
										</p>
									</div>
				...
				<div
					class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-warm-dark-900/50 rounded-xl p-5 border border-gray-100 dark:border-warm-border/30"
				>
			...
			<div v-if="activeTab === 'jewelry'" class="space-y-6">
				...
				>
					...
					<div class="grid grid-cols-3 sm:grid-cols-5 gap-2 mb-4">
						<button
							...
							:class="
								form.item_type === type.name
									? 'border-[#D4AF37] bg-[#D4AF37]/5 text-[#D4AF37] font-bold shadow-sm'
									: 'border-gray-100 dark:border-gray-800/80 bg-white dark:bg-warm-dark-900 text-gray-600 dark:text-gray-300 hover:bg-gray-50/50 dark:hover:bg-warm-dark-800/30'
							"
						>
							<span
								v-html="type.icon"
								class="w-6 h-6 flex items-center justify-center text-gray-400 dark:text-gray-500"
								:class="{
				...
				<div
					class="bg-gray-50 dark:bg-warm-dark-900/50 rounded-xl p-5 border border-gray-100 dark:border-warm-border/30 space-y-4"
				>
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
						<!-- Item Weight -->
						<div>
							...
							<div class="relative">
								...
								/>
								<span class="text-xs text-gray-400 absolute right-3 top-2.5"
									>g</span
								>
							</div>
						</div>
					...
					<div
						...
						class="p-3.5 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/20 rounded-lg flex items-start gap-3"
					>
						<svg
						...
						<div>
							<span class="text-xs font-bold text-amber-800 dark:text-amber-400"
								>Precision Intake Alert</span
							>
							<p class="text-[11px] text-amber-700 dark:text-amber-500 mt-0.5">
								Metal type is selected but gross weight is missing. Documenting
								exact gram weight protects your store against weight-swap claims at
								pickup.
							</p>
						</div>
					</div>
					...
					<div>
						<label
							class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2"
							>Quick Intake Condition Checklist</label
						>
					...
					<div>
						<label
							...
							>Detailed Item Condition Description</label
						>
						<textarea
				...
				<!-- Gemstones Section -->
				<div
					class="bg-purple-50/50 dark:bg-purple-900/10 rounded-xl p-5 border border-purple-100 dark:border-purple-900/20"
				>
					<div class="flex items-center justify-between mb-3">
						<div class="flex items-center gap-2">
							<span class="text-xl">💎</span>
							<h4 class="text-sm font-bold text-purple-800 dark:text-purple-400">
								Gemstone Breakdown Checklist
							</h4>
						</div>
						<button
							type="button"
					...
					<p
						...
						class="text-xs text-purple-500/80 dark:text-purple-400/60 text-center py-6 bg-white dark:bg-warm-dark-900 rounded-lg border border-purple-50 dark:border-purple-950/20"
					>
					...
					<div v-else class="space-y-3 max-h-60 overflow-y-auto custom-scrollbar pr-1">
						...
						>
							<div
								class="flex items-center justify-between border-b border-gray-50 dark:border-gray-800 pb-2 mb-2.5"
							>
							...
							<div class="grid grid-cols-2 sm:grid-cols-5 gap-2">
								...
								<div>
									<label
										class="block text-[10px] font-bold text-gray-400 uppercase mb-0.5"
										>Type *</label
									>
									<select
			...
			<div v-if="activeTab === 'pricing'" class="space-y-6">
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						<div class="flex flex-col h-full space-y-3">
							...
							<div class="grid grid-cols-12 gap-3 h-52">
								...
								<div
									class="col-span-4 overflow-y-auto pr-1 border-r border-gray-100 dark:border-gray-800 space-y-1 scrollbar-thin"
								>
									<button
										type="button"
								...
								<div
									class="col-span-8 overflow-y-auto p-1 bg-white dark:bg-warm-dark-900/20 border border-gray-200 dark:border-gray-700/60 rounded-xl space-y-1"
								>
									<!-- Smart Recommendations Section -->
									<div
										v-if="recommendedRepairTypes.length > 0"
										class="mb-3 pb-3 border-b border-gray-100 dark:border-warm-border/30"
									>
										<div
											class="flex items-center gap-1.5 px-2 py-1.5 mb-1 bg-amber-50/40 dark:bg-amber-950/5 rounded-lg border border-amber-200/20 dark:border-amber-900/10"
										>
											<span class="text-xs">✨</span>
											<span
												class="text-[10px] font-extrabold uppercase tracking-wider text-amber-500"
												>Smart Suggestions</span
											>
											<span
												class="text-[8px] bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-1.5 py-0.5 rounded-full font-bold"
												>Based on Intake Checklist</span
											>
										</div>
										<div class="space-y-1">
											<button
											...
											>
												<div class="pr-2 truncate">
													...
													>
														<span
															v-if="form.repair_type === rt.name"
															class="text-amber-500 font-extrabold flex-shrink-0"
															>✓</span
														>
														<span
															class="font-bold text-[11px] group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors"
															>{{ rt.repair_name || rt.name }}</span
														>
													</div>
													<div
														class="text-[9px] text-gray-400 font-normal uppercase tracking-wider mt-0.5"
													>
														{{ rt.category || 'General' }}
													</div>
												</div>
												<div class="text-right flex-shrink-0">
													<span
														class="font-bold font-mono text-[11px] text-gray-900 dark:text-gray-100 bg-white dark:bg-warm-dark-900 px-1.5 py-0.5 border border-gray-100 dark:border-gray-800 rounded-md"
														>${{ rt.base_price || 0 }}</span
													>
													<div
														v-if="rt.estimated_days"
														class="text-[8px] text-gray-400 font-normal mt-0.5"
													>
														{{ rt.estimated_days }}d turnaround
													</div>
												</div>
											</button>
										</div>
									...
									<!-- Empty State -->
									<div
										v-if="filteredRepairTypes.length === 0"
							...
							>
								<div class="flex justify-between items-center">
									...
									<span class="font-bold text-[#D4AF37]">{{
										selectedRepairType.repair_name || selectedRepairType.name
									}}</span>
								</div>
								<div
									class="grid grid-cols-2 gap-2 pt-1 border-t border-gray-100 dark:border-gray-800/80"
								>
									<div class="flex justify-between items-center">
										<span class="text-gray-400">Standard Price:</span>
										<span
											class="font-bold text-gray-900 dark:text-gray-100 font-mono"
									...
									<div class="flex justify-between items-center">
										...
										<span class="font-bold text-gray-900 dark:text-gray-100"
											>{{
												selectedRepairType.estimated_days || 3
											}}
											days</span
										>
									</div>
								...
								<p
									...
									class="text-[10px] text-gray-400 pt-1 border-t border-gray-100 dark:border-gray-800/80 italic"
								>
						...
						<div>
							<label
								class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1"
								>Detailed Description of Work</label
							>
				...
				>
					...
					<div>
						...
						<div class="space-y-3">
							...
							<div
								class="flex items-center justify-between border-t border-gray-200/50 pt-2"
							>
					...
					<div class="border-l border-gray-200/40 pl-0 md:pl-5 space-y-3.5">
						...
						<label class="flex items-center gap-2 cursor-pointer">
							<input
								...
								type="checkbox"
								class="rounded border-gray-300 dark:border-gray-700 text-[#D4AF37] focus:ring-[#D4AF37]"
							/>
							<span class="text-xs font-bold text-gray-700 dark:text-gray-300"
								>Collect Intake Deposit (Standard 50%)</span
							>
						</label>

						<div
							v-if="form.collect_deposit"
							class="p-3 bg-white dark:bg-warm-dark-900 rounded-xl border border-gray-100 dark:border-gray-800 space-y-2"
						>
							<div class="flex justify-between items-center">
								<label class="text-xs text-gray-500 font-medium"
									>Recommended (50%):</label
								>
								<span
									class="text-xs font-bold font-mono text-gray-800 dark:text-gray-200"
									>${{ recommendedDeposit }}</span
								>
							</div>
							...
							<div class="grid grid-cols-2 gap-2">
								<div class="relative">
									...
									/>
									<span class="text-[10px] text-gray-400 absolute left-2 top-2"
										>$</span
									>
								</div>
								<select
							...
							<div
								...
								class="text-[10px] text-gray-400 text-right font-mono"
							>
				...
				<div
					class="bg-orange-50/50 dark:bg-orange-950/10 rounded-xl p-5 border border-orange-100 dark:border-orange-950/20 space-y-3"
				>
					<div class="flex items-center gap-2">
						<span class="text-lg">🔒</span>
						<h4 class="text-sm font-bold text-orange-800 dark:text-orange-400">
							Customer ID Verification (JVC Compliance)
						</h4>
					</div>
					<p class="text-[10px] text-orange-600 dark:text-orange-500">
						AML and Patriot Act guidelines require valid identification verification
						for high-value custom work or precious metal custody.
					</p>
				...
				<div
					class="bg-gray-100 dark:bg-warm-dark-900/80 rounded-xl p-4 border border-gray-200/50 dark:border-gray-800 text-xs text-gray-500 space-y-3"
				>
					<p class="font-bold text-gray-700 dark:text-gray-300">
						Liability Waiver & Custom Repair Disclaimers
					</p>
					<div
						class="max-h-20 overflow-y-auto custom-scrollbar border-l-2 border-[#D4AF37] pl-3 leading-relaxed text-[10px] dark:text-gray-400"
					>
						Zevar Jewelers makes every reasonable effort to safely clean and service
						your precious items. Clients agree that precious gemstones and prongs can
						become naturally brittle or fracture during stone resetting or sizing
						processes. Zevar Jewelers is not liable for structural stone fracturing
						inherent to stones during bench work. Leftover metal scrap remains bench
						property unless scrap pickup is explicitly requested in writing. Items
						unclaimed for more than 90 days from the date of promised completion are
						subject to private sale or salvage to cover estimated service costs.
					</div>
					<label class="flex items-center gap-2.5 cursor-pointer pt-1">
						<input
							v-model="form.intake_checklist_signed"
							type="checkbox"
							class="rounded border-gray-300 dark:border-gray-700 text-[#D4AF37] focus:ring-[#D4AF37]"
						/>
						<span class="font-bold text-gray-700 dark:text-gray-300"
							>Customer has read and signs off on the estimate, disclosures, and
							waivers</span
						>
					</label>
				</div>
		...
		if (!form.value.repair_type && recommendedRepairTypes.value.length > 0) {
			...
			if (recommendedRepairTypes.value.length === 1) {
				...
				toast({
					...
					message: `Selected "${
						recommendedRepairTypes.value[0].repair_name ||
						recommendedRepairTypes.value[0].name
					}" based on Quick Intake checklist`,
					icon: 'check',
					intent: 'info',
				})
			}
...
const priorityOptions = [
	{
		...
		activeClass:
			'border-gray-400 bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 font-extrabold shadow-sm',
	},
	{
		value: 'Medium',
		bulletClass: 'bg-yellow-400',
		activeClass:
			'border-yellow-400 bg-yellow-50 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300 font-extrabold shadow-sm',
	},
	{
		value: 'High',
		bulletClass: 'bg-orange-400',
		activeClass:
			'border-orange-400 bg-orange-50 dark:bg-orange-900 text-orange-700 dark:text-orange-300 font-extrabold shadow-sm',
	},
	{
		value: 'Urgent',
		bulletClass: 'bg-red-400',
		activeClass:
			'border-red-500 bg-red-50 dark:bg-red-900 text-red-700 dark:text-red-300 font-extrabold shadow-sm',
	},
]
...
const itemTypes = [
	{
		name: 'Ring',
		icon: `<svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="14" r="6" /><path d="M12 8l-2-3h4z" stroke-linecap="round" stroke-linejoin="round" /></svg>`,
	},
	{
		name: 'Necklace',
		icon: `<svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M6 4c0 6 3 10 6 10s6-4 6-10" stroke-linecap="round" /><circle cx="12" cy="15" r="1.5" fill="currentColor" /></svg>`,
	},
	{
		name: 'Bracelet',
		icon: `<svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><ellipse cx="12" cy="12" rx="8" ry="4" /><path d="M6 12a2 2 0 0 1 4 0m4 0a2 2 0 0 1 4 0" stroke-linecap="round" /></svg>`,
	},
	{
		name: 'Earring',
		icon: `<svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 4v4m0 0l-3 3.5 3 3.5 3-3.5-3-3.5z" stroke-linecap="round" stroke-linejoin="round" /></svg>`,
	},
	{
		name: 'Watch',
		icon: `<svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="5" /><path d="M12 7V3h-2h4M12 17v4h-2h4" stroke-linecap="round" stroke-linejoin="round" /></svg>`,
	},
	{
		name: 'Chain',
		icon: `<svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 15l6-6M8 10a3 3 0 0 1 4 4M12 10a3 3 0 0 1 4 4" stroke-linecap="round" stroke-linejoin="round" /></svg>`,
	},
	{
		name: 'Pendant',
		icon: `<svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M6 5l6 7 6-7" stroke-linecap="round" /><path d="M12 12c-1.5 2-1.5 4 0 5.5s3 1.5 3 0-1.5-3.5-3-5.5z" fill="currentColor" /></svg>`,
	},
	{
		name: 'Brooch',
		icon: `<svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="4" /><path d="M12 2v6M12 16v6M2 12h6M16 12h6" stroke-linecap="round" /></svg>`,
	},
	{
		name: 'Other',
		icon: `<svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 3l1.5 4.5L18 9l-4.5 1.5L12 15l-1.5-4.5L6 9l4.5-1.5z" stroke-linecap="round" stroke-linejoin="round" /></svg>`,
	},
]
...
const recommendedRepairTypes = computed(() => {
	...
	return allRepairTypes.value.filter((rt) => {
		...
		return Array.from(keywords).some(
			(kw) => name.includes(kw) || desc.includes(kw) || cat.includes(kw)
		)
	})
})
...
const filteredRepairTypes = computed(() => {
	return repairTypes.value.filter((rt) => {
		const matchCategory = !selectedCategory.value || rt.category === selectedCategory.value
		const matchSearch =
			!repairTypeSearch.value ||
			(rt.repair_name || rt.name || '')
				.toLowerCase()
				.includes(repairTypeSearch.value.toLowerCase()) ||
			(rt.description || '').toLowerCase().includes(repairTypeSearch.value.toLowerCase())
		return matchCategory && matchSearch
	})
})
...
async function submit() {
	...
	try {
		...
		if (form.value.gemstones.length > 0) {
			form.value.stone_weight = form.value.gemstones.reduce(
				(sum, item) => sum + (parseFloat(item.carat_weight) || 0),
				0
			)
		}
		...
		const payload = {
			...
			deposit_payment_method: form.value.collect_deposit
				? form.value.deposit_payment_method
				: undefined,
		}
