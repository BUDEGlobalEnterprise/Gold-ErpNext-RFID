<template>
	<Teleport to="body">
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="$emit('close')">
				...
				<div class="p-6">
					<div class="flex items-center justify-between mb-4">
						...
						<div class="flex gap-2">
							<button @click="showRepairTypeManager = true" class="px-3 py-1 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200">
								Manage Repair Types
							</button>
						</div>
					</div>
					...
					<form @submit.prevent="submit" class="space-y-4">
						...
						<div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
							<div class="flex items-center justify-between mb-2">
								...
								<button type="button" @click="showNewCustomerModal = true" class="text-xs px-2 py-1 bg-[#D4AF37] text-black rounded hover:bg-[#c9a432] flex items-center gap-1">
									<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
									New Customer
								</button>
							</div>
							<div class="relative">
								<input v-model="customerSearch" type="text" placeholder="Search customer by name or phone..." @input="searchCustomers"
									class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-900 text-sm" />
								<div v-if="customerResults.length > 0" class="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border rounded-lg shadow-lg max-h-48 overflow-y-auto">
									<button v-for="c in customerResults" :key="c.name" type="button" @click="selectCustomer(c)"
							...
							<p v-if="form.customer" class="mt-2 text-xs text-green-600 flex items-center gap-2">
								...
								<button v-if="customerRepairHistory.length > 0" type="button" @click="showHistory = !showHistory" class="text-blue-600 hover:underline">
									{{ showHistory ? 'Hide' : 'View' }} History ({{ customerRepairHistory.length }})
								</button>
							</p>

							<!-- Customer Repair History -->
							<div v-if="showHistory && customerRepairHistory.length > 0" class="mt-3 p-2 bg-white dark:bg-gray-900 rounded text-xs">
								<p class="font-medium text-gray-700 mb-2">Previous Repairs</p>
								<div class="space-y-1 max-h-24 overflow-y-auto">
									<div v-for="h in customerRepairHistory" :key="h.name" class="flex justify-between text-gray-600">
						...
						<div>
							...
							<input v-model="form.customer_phone" type="tel" placeholder="(555) 123-4567"
						...
						<div>
							...
							<div class="flex gap-2 mb-2">
								<select v-model="selectedCategory" @change="filterRepairTypes" class="px-3 py-1 border rounded-lg bg-white dark:bg-gray-800 text-sm">
									<option value="">All Categories</option>
									<option v-for="cat in repairCategories" :key="cat" :value="cat">{{ cat }}</option>
								</select>
								<input v-model="repairTypeSearch" @input="filterRepairTypes" type="text" placeholder="Search repair types..." class="flex-1 px-3 py-1 border rounded-lg bg-white dark:bg-gray-800 text-sm" />
							</div>
							<select v-model="form.repair_type" required class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-800 text-sm max-h-40">
								<option value="">Select...</option>
								<optgroup v-for="(types, category) in groupedRepairTypes" :key="category" :label="category">
									<option v-for="rt in types" :key="rt.name" :value="rt.name">
										{{ rt.repair_name || rt.name }} - ${{ rt.base_price || 0 }}
									</option>
								</optgroup>
							</select>
							<div v-if="selectedRepairType" class="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded text-xs">
								...
								<p v-if="selectedRepairType.description" class="mt-1 text-gray-500">{{ selectedRepairType.description }}</p>
							</div>
						</div>
						...
						<div v-if="form.repair_type">
							...
							<div class="flex flex-wrap gap-1 mb-2">
								<button v-for="(template, idx) in noteTemplates" :key="idx" type="button" @click="addNoteTemplate(template)"
						...
						<div class="grid grid-cols-2 gap-3">
							<div>
								...
								<select v-model="form.item_type" class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-800 text-sm">
									<option value="">Select...</option>
									<option>Ring</option><option>Necklace</option><option>Bracelet</option>
									<option>Earring</option><option>Watch</option><option>Chain</option>
									<option>Pendant</option><option>Brooch</option><option>Other</option>
								</select>
							</div>
							<div>
						...
						<div class="grid grid-cols-2 gap-3">
							<div>
								...
								<input v-model="form.item_brand" type="text" placeholder="e.g., Rolex, Tiffany"
									class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-800 text-sm" />
							</div>
							<div>
								<label class="block text-sm font-medium mb-1">Serial Number</label>
								<input v-model="form.serial_number" type="text" placeholder="For watches, etc."
									class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-800 text-sm" />
							</div>
						</div>
						...
						<div>
							...
							<textarea v-model="form.item_description" rows="2" placeholder="Describe the item and repair needed..."
						...
						<div class="grid grid-cols-2 gap-3">
							<div>
								...
								<input v-model.number="form.item_weight" type="number" step="0.01" placeholder="0.00"
									class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-800 text-sm" />
							</div>
							<div>
								<label class="block text-sm font-medium mb-1">Stone Weight (ct)</label>
								<input v-model.number="form.stone_weight" type="number" step="0.01" placeholder="0.00"
									class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-800 text-sm" />
							</div>
						</div>
						...
						<div>
							...
							<textarea v-model="form.item_condition" rows="1" placeholder="Scratches, worn prongs, loose stones, etc."
								class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-800 text-sm resize-none"></textarea>
						</div>
						...
						<div class="grid grid-cols-2 gap-3">
							...
							<div>
								...
								<select v-model="form.purity" class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-800 text-sm">
									<option value="">Select...</option>
									<option>10K</option><option>14K</option><option>18K</option><option>22K</option><option>24K</option>
									<option>925</option><option>950</option><option>999</option>
								</select>
							</div>
						</div>
						...
						<div class="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-3 border border-purple-100 dark:border-purple-800/30">
							<div class="flex items-center justify-between mb-2">
								...
								<button type="button" @click="addGemstone" class="text-xs px-2 py-1 bg-purple-500 text-white rounded hover:bg-purple-600">+ Add Stone</button>
							</div>
								...
								<div v-for="(stone, idx) in form.gemstones" :key="idx" class="bg-white dark:bg-gray-800 rounded p-2 text-xs">
									<div class="grid grid-cols-3 gap-2">
										<select v-model="stone.type" class="px-2 py-1 border rounded bg-white dark:bg-gray-900">
											...
											<option>Diamond</option><option>Ruby</option><option>Sapphire</option>
											<option>Emerald</option><option>Pearl</option><option>Other</option>
										</select>
									...
									<div class="grid grid-cols-3 gap-2 mt-1">
										...
										<button type="button" @click="removeGemstone(idx)" class="text-red-500 hover:text-red-700">Remove</button>
									</div>
								</div>
							</div>
						</div>
						...
						<div>
							...
							<input v-model.number="form.estimated_cost" type="number" step="0.01" min="0" placeholder="0.00"
								class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-800 text-sm" />
						</div>
						...
						<div class="flex gap-3 pt-2">
							<button type="button" @click="$emit('close')" class="flex-1 py-2 border rounded-lg text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-800">Cancel</button>
							<button type="submit" :disabled="submitting" class="flex-1 py-2 bg-[#D4AF37] text-black rounded-lg text-sm font-bold hover:bg-[#c9a432] disabled:opacity-50">
			...
			<!-- New Customer Modal -->
			<CustomerCreationModal v-if="showNewCustomerModal" @close="showNewCustomerModal = false" @created="onCustomerCreated" />

			<!-- Repair Type Manager Modal -->
			<RepairTypeManager v-if="showRepairTypeManager" @close="showRepairTypeManager = false" @created="onRepairTypeCreated" />
		</div>
	</Teleport>
</template>
...
<script setup>
	...
	const groupedRepairTypes = computed(() => {
		const groups = {}
		const filtered = selectedCategory.value || repairTypeSearch.value
			? repairTypes.value.filter(rt => {
				const matchCategory = !selectedCategory.value || rt.category === selectedCategory.value
				const matchSearch = !repairTypeSearch.value ||
					(rt.repair_name || rt.name || '').toLowerCase().includes(repairTypeSearch.value.toLowerCase()) ||
	...
	function addGemstone() {
		form.value.gemstones.push({ type: '', count: 1, carat_weight: null, color: '', clarity: '', setting_type: '', is_treated: false })
	}
	...
	function onCustomerCreated(customer) {
		...
		toast({ title: 'Customer Selected', message: `${customer.customer_name} selected`, icon: 'check', intent: 'success' })
	}
	...
	async function submit() {
		if (!form.value.customer) {
			toast({ title: 'Missing Information', message: 'Please select a customer', icon: 'alert-circle', intent: 'error' })
			return
		}
		if (!form.value.repair_type) {
			toast({ title: 'Missing Information', message: 'Please select a repair type', icon: 'alert-circle', intent: 'error' })
			return
		}
