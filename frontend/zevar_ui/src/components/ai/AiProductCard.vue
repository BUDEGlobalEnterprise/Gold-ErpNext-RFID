  ...
  <div class="ai-product" @click="$emit('click', product)">
    ...
    <div class="ai-product__info">
      ...
      <div class="ai-product__tags">
        ...
        <span v-if="product.jewelry_type" class="ai-tag ai-tag--primary">{{ product.jewelry_type }}</span>
      </div>
      <div class="ai-product__footer">
        <span v-if="product.msrp" class="ai-product__price">${{ product.msrp.toLocaleString() }}</span>
        <span class="ai-product__stock" :class="{ 'ai-product__stock--low': product.stock_qty <= 2 }">
          {{ product.stock_qty > 0 ? `${product.stock_qty} in stock` : 'Out of stock' }}
        </span>
      </div>
      <div class="ai-product__match">
