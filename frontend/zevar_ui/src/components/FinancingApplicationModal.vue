		...
		<div class="p-6" style="min-height: 400px">
			...
			<template v-if="step === 'info'">
				...
				<p class="text-sm text-gray-500 dark:text-gray-400 mb-6">
					We'll try each provider in order until you're approved.
				</p>
				...
				<div class="grid grid-cols-2 gap-4 mb-4">
					...
					<div>
						...
						<input
							...
							type="text"
							maxlength="4"
							class="w-full px-3 py-2 bg-white dark:bg-[#0F1115] border border-gray-200 dark:border-warm-border rounded-lg text-sm font-mono"
						/>
					</div>
				</div>
				<div class="mb-4">
					...
					<input
						...
						class="w-full px-3 py-2 bg-white dark:bg-[#0F1115] border border-gray-200 dark:border-warm-border rounded-lg text-sm mb-2"
					/>
					<div class="grid grid-cols-3 gap-2">
						<input
							...
							type="text"
							placeholder="City"
							class="px-3 py-2 bg-white dark:bg-[#0F1115] border border-gray-200 dark:border-warm-border rounded-lg text-sm"
						/>
						<input
							v-model="form.state"
							type="text"
				...
				<div
					v-if="error"
					class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/30 rounded-xl p-3 mb-4"
				>
					<p class="text-sm text-red-600 dark:text-red-400">{{ error }}</p>
				</div>

				<button
					@click="submitWaterfall"
					...
					:class="
						!canSubmitInfo || submitting
							? 'bg-gray-100 text-gray-400 dark:bg-warm-dark-700 dark:text-gray-600'
							: 'bg-gray-900 text-white dark:bg-[#D4AF37] dark:text-black'
					"
				>
					<span
						v-if="submitting"
			...
			<template v-if="step === 'results'">
				<h2 class="text-xl font-bold text-gray-900 dark:text-white mb-4">
					Financing Application Results
				</h2>
				<div class="space-y-3 mb-6">
					<div
						v-for="result in waterfallResults"
						:key="result.provider"
						...
						:class="{
							...
							'border-gray-200 bg-gray-50 dark:bg-warm-dark-700 dark:border-warm-border':
					...
					>
						<div class="flex items-center gap-3">
							...
							>
								...
								<svg
									v-else-if="result.status === 'denied'"
									class="w-4 h-4"
									fill="none"
									stroke="currentColor"
									viewBox="0 0 24 24"
							...
							<div>
								...
								<span
									v-if="result.status === 'approved' && result.approval_amount"
				...
				<div
					v-if="approvedResult"
					class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800/30 rounded-xl p-5 mb-4 text-center"
				>
					<h3 class="text-lg font-bold text-green-700 dark:text-green-400 mb-1">
						Approved by {{ approvedResult.provider }}!
					</h3>
					<p class="text-2xl font-bold text-green-700 dark:text-green-400">
						{{ formatCurrency(approvedResult.approval_amount) }}
					</p>
					<p class="text-sm text-green-600 dark:text-green-500 mt-1">
						Application ID: {{ approvedResult.application_id }}
					</p>
				</div>

				<div
					v-else-if="waterfallResults.length > 0"
					class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/30 rounded-xl p-5 mb-4 text-center"
				>
					<h3 class="text-lg font-bold text-red-700 dark:text-red-400 mb-1">
						Not Approved
					</h3>
					<p class="text-sm text-red-600 dark:text-red-500">
						All financing providers were unable to approve at this time.
					</p>
				</div>

				<button
					@click="close"
					class="w-full py-3 rounded-xl font-bold bg-gray-900 dark:bg-[#D4AF37] text-white dark:text-black hover:bg-gray-800 dark:hover:bg-[#b5952f] transition"
				>
					Done
				</button>
			</template>
