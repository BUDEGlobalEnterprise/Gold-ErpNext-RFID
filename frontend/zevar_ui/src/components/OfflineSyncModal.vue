	...
	<BaseModal :show="show" max-width="max-w-2xl" :show-close="true" @close="$emit('close')">
		<div class="p-6 font-sans">
			...
			<div class="flex items-center gap-3 mb-6">
				<div class="w-10 h-10 rounded-full bg-[#D4AF37]/10 flex items-center justify-center border border-[#D4AF37]/30">
			...
			<!-- Status Bar -->
			<div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-warm-dark-700/20 border border-gray-100 dark:border-warm-border rounded-xl mb-6 text-xs">
				<div class="flex items-center gap-4">
					<div class="flex items-center gap-1.5 font-bold uppercase">
						<span class="relative flex h-2.5 w-2.5">
							<span v-if="offline.isOnline" class="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
							<span class="relative inline-flex rounded-full h-2.5 w-2.5" :class="offline.isOnline ? 'bg-green-500' : 'bg-red-500'"></span>
						</span>
						<span :class="offline.isOnline ? 'text-green-600 dark:text-green-400' : 'text-red-500'">
							{{ offline.isOnline ? 'Online' : 'Offline Mode' }}
						</span>
					</div>
					<div class="text-gray-400 dark:text-gray-500 font-mono">
						Last Sync: {{ offline.lastSyncTime ? formatTime(offline.lastSyncTime) : 'Never' }}
					</div>
				</div>
				...
				>
					<svg v-if="syncing" class="w-3.5 h-3.5 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<circle cx="12" cy="12" r="10" stroke-width="3" class="opacity-25"></circle>
						<path stroke-width="3" class="opacity-75" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
					</svg>
			...
			<!-- Tabs -->
			<div class="flex border-b border-gray-100 dark:border-warm-border/50 mb-4 text-xs font-bold uppercase tracking-wider">
				<button
					@click="activeTab = 'failed'"
					class="pb-2.5 px-4 border-b-2 transition"
					:class="activeTab === 'failed' ? 'border-red-500 text-red-600 dark:text-red-400' : 'border-transparent text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'"
				>
				...
				<button
					...
					class="pb-2.5 px-4 border-b-2 transition"
					:class="activeTab === 'conflicts' ? 'border-amber-500 text-amber-600 dark:text-amber-400' : 'border-transparent text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'"
				>
				...
				<button
					...
					class="pb-2.5 px-4 border-b-2 transition"
					:class="activeTab === 'pending' ? 'border-[#D4AF37] text-[#D4AF37]' : 'border-transparent text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'"
				>
				...
				<button
					...
					class="pb-2.5 px-4 border-b-2 transition"
					:class="activeTab === 'synced' ? 'border-green-500 text-green-600 dark:text-green-400' : 'border-transparent text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'"
				>
			...
			<div class="min-h-[250px] max-h-[400px] overflow-y-auto custom-scrollbar pr-1 text-sm">
				...
				<div v-if="loading" class="py-12 text-center">
					<div class="animate-spin rounded-full h-6 w-6 border-2 border-[#D4AF37] border-t-transparent mx-auto mb-3"></div>
				...
				<div v-else>
					...
					<div v-if="activeTab === 'failed'">
						<div v-if="failedItems.length === 0" class="py-12 text-center text-gray-400 text-xs">
						...
						<div v-else class="space-y-4">
							...
							>
								...
								<div class="flex items-start justify-between">
									<div>
										<span class="px-2 py-0.5 rounded bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 text-[10px] font-bold uppercase tracking-wider">
											{{ item.mode }} Transaction
										</span>
										<span class="text-xs font-bold font-mono text-gray-700 dark:text-gray-300 ml-2">
											ID: {{ item.id }}
										</span>
									</div>
								...
								<!-- Details -->
								<div class="grid grid-cols-2 gap-4 text-xs font-medium py-1.5 border-t border-b border-gray-100 dark:border-warm-border/30">
									<div>
										<span class="text-gray-400 block">Amount:</span>
										<span class="font-mono text-gray-900 dark:text-white font-bold text-sm">
								...
								<div class="p-3 bg-red-500/5 border border-red-500/20 rounded-lg text-xs leading-relaxed">
									<p class="font-bold text-red-600 dark:text-red-400 mb-1 flex items-center gap-1">
								...
								<div class="text-xs">
									...
									<div v-if="isExpanded(item.id)" class="mt-2 p-2.5 bg-gray-900 dark:bg-black text-gray-300 font-mono text-[10px] rounded border dark:border-warm-border overflow-x-auto whitespace-pre-wrap max-h-36">
										{{ item.error || 'No traceback log details available.' }}
									</div>
								</div>

								<!-- Controls -->
								<div class="flex items-center justify-end gap-2 pt-2 border-t border-gray-100 dark:border-warm-border/30">
									<button
					...
					<div v-if="activeTab === 'conflicts'">
						<div v-if="conflictItems.length === 0" class="py-12 text-center text-gray-400 text-xs">
						...
						<div v-else class="space-y-4">
							...
							>
								...
								<div class="flex items-start justify-between">
									<div>
										<span class="px-2 py-0.5 rounded bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 text-[10px] font-bold uppercase tracking-wider">
											{{ item.mode }} Conflict
										</span>
										<span class="text-xs font-bold font-mono text-gray-700 dark:text-gray-300 ml-2">
											ID: {{ item.id }}
								...
								<div class="grid grid-cols-2 gap-4 text-xs font-medium py-1.5 border-t border-b border-gray-100 dark:border-warm-border/30">
									...
									<div>
										...
										<span class="text-amber-600 dark:text-amber-400 uppercase font-black tracking-wider text-[10px]">
								...
								<!-- Human Message -->
								<div class="p-3 bg-amber-500/5 border border-amber-500/20 rounded-lg text-xs leading-relaxed">
									<p class="font-bold text-amber-600 dark:text-amber-400 mb-1 flex items-center gap-1">
					...
					<div v-if="activeTab === 'pending'">
						<div v-if="pendingItems.length === 0" class="py-12 text-center text-gray-400 text-xs">
						...
						<div v-else class="space-y-4">
							...
							>
								...
								<div class="flex items-start justify-between">
									<div>
										<span class="px-2 py-0.5 rounded bg-gray-200 text-gray-700 dark:bg-warm-dark-600 dark:text-gray-300 text-[10px] font-bold uppercase tracking-wider">
											{{ item.mode }} Queued
										</span>
										<span class="text-xs font-bold font-mono text-gray-700 dark:text-gray-300 ml-2">
											ID: {{ item.id }}
								...
								<div class="text-xs text-gray-500 dark:text-gray-400 flex items-center justify-between">
									<span>Attempts: {{ item.attempts || 0 }}/{{ item.max_attempts || 5 }}</span>
					...
					<div v-if="activeTab === 'synced'">
						<div v-if="syncedItems.length === 0" class="py-12 text-center text-gray-400 text-xs">
						...
						<div v-else class="space-y-4">
							...
							>
								...
								<div class="flex items-start justify-between">
									<div>
										<span class="px-2 py-0.5 rounded bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 w-fit">
											<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
											{{ item.mode }} Synced
										</span>
										<span class="text-xs font-bold font-mono text-gray-700 dark:text-gray-300 ml-1 mt-1 block">
											ID: {{ item.id }}
										</span>
									</div>
									<div class="text-right">
										...
										<span class="text-[11px] text-green-600 dark:text-green-400 font-medium">
											Synced: {{ formatTime(item.synced_at || item.created_at) }}
										</span>
									</div>
								</div>

								<!-- Details -->
								<div class="grid grid-cols-2 gap-4 text-xs font-medium py-1.5 border-t border-gray-100 dark:border-warm-border/30">
									<div>
										<span class="text-gray-400 block">Amount:</span>
										<span class="font-mono text-gray-900 dark:text-white font-bold text-sm">
											${{ getAmount(item).toFixed(2) }}
...
function translateError(errorStr) {
	if (!errorStr) return 'An unknown server or network error occurred during transaction synchronization.'
	const err = String(errorStr).toLowerCase()

	if (err.includes('not available') || err.includes('out of stock') || err.includes('insufficient') || err.includes('reserved for another') || err.includes('already been sold') || err.includes('serial no')) {
	...
	if (err.includes('csrf') || err.includes('session expired') || err.includes('login') || err.includes('not permitted') || err.includes('guest') || err.includes('authenticated') || err.includes('verification failed')) {
	...
	if (err.includes('failed to fetch') || err.includes('network') || err.includes('timeout') || err.includes('dns') || err.includes('502') || err.includes('503') || err.includes('504')) {
	...
	if (err.includes('duplicate') || err.includes('already exists') || err.includes('primary key') || err.includes('integrity')) {
	...
	if (err.includes('tax') || err.includes('pricing') || err.includes('rate') || err.includes('rounding')) {
...
async function deleteItem(id) {
	if (confirm('Are you sure you want to discard this transaction? It will be deleted from your offline queue permanently.')) {
...
function retryConflictModified(item) {
	alert('To modify and resolve this conflict, please recall the item or checkout modified quantities.')
}
</script>
