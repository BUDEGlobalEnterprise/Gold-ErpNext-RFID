<template>
	...
	>
		<div
			class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto"
		>
			<!-- Header -->
			<div
				class="px-6 py-4 border-b border-red-200 dark:border-red-800/30 bg-red-50 dark:bg-red-900/20 rounded-t-xl"
			>
				<div class="flex items-center gap-3">
					<div
						class="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/40 flex items-center justify-center"
					>
						<svg
			...
			<div class="px-6 py-4 space-y-4">
				...
				<div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
					...
					<p class="text-sm text-gray-600 dark:text-gray-400">
						{{ conflict.server_error || 'Unknown conflict' }}
					</p>
				</div>
			...
			>
				...
				<button
					...
					:class="
						selectedResolution === 'resolved_cancel'
...
const conflictItems = computed(() => {
	try {
		...
		const items =
			typeof payload.items === 'string' ? JSON.parse(payload.items) : payload.items || []
		return items
	} catch {
		return []
	}
...
const resolutionOptions = computed(() => {
	const options = [
		{
			value: 'resolved_cancel',
			label: 'Cancel entire order',
			description: 'Remove this offline order completely. No charges will be processed.',
		},
	]
	...
	if (conflictItems.value.length > 1) {
		options.push({
			...
			description:
				'Remove the conflicting item from the invoice and sync the remaining items.',
		})
	}

	options.push(
		{
			value: 'resolved_substitute',
			label: 'Substitute with similar item',
			description: 'Replace the unavailable item with another piece from inventory.',
		},
		{
			value: 'escalated',
			label: 'Escalate to manager',
			description: 'Flag for manager review. The order stays in conflict queue.',
		}
	)
