	...
	<BaseModal :show="true" max-width="max-w-md" :show-close="true" @close="$emit('close')">
		<div class="p-6">
			...
			<div
				...
				class="mb-5 max-h-32 overflow-y-auto custom-scrollbar"
			>
				<div
				...
				>
					...
					<span class="text-gray-400 dark:text-gray-500 truncate max-w-[140px]">{{
						m.reason
					}}</span>
				</div>
			</div>
			...
			<div class="space-y-4">
				...
				<div
					class="flex rounded-lg border border-gray-200 dark:border-white/10 overflow-hidden"
				>
					<button
						@click="form.movement_type = 'Cash In'"
						class="flex-1 py-2 text-sm font-bold transition"
						:class="
							form.movement_type === 'Cash In'
				...
				<div v-if="form.movement_type === 'Cash Out' && form.amount > 100">
					<label
						class="block text-xs font-bold text-amber-500 uppercase tracking-wider mb-1.5"
						>Manager PIN Required</label
					>
					<input
