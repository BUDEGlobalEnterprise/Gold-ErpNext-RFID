	...
	>
		...
		<div class="flex justify-between items-start mb-3">
			<div>
				...
				<span class="ml-2 inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold" :class="getStatusBadgeClass(order.status)">
					{{ order.status }}
				</span>
			</div>
			<div class="flex items-center gap-1">
				...
				<span v-if="order.is_warranty_repair" class="text-green-500" title="Warranty Repair">
		...
		<div class="flex flex-wrap gap-1 mb-2">
			<span v-if="order.item_type" class="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-[10px] text-gray-600">
				{{ order.item_type }}
			</span>
			<span v-if="order.stone_weight && order.stone_weight > 0" class="px-1.5 py-0.5 bg-purple-100 dark:bg-purple-900/30 rounded text-[10px] text-purple-600">
				💎 {{ order.stone_weight }}ct
			</span>
			<span v-if="order.item_brand" class="px-1.5 py-0.5 bg-amber-100 dark:bg-amber-900/30 rounded text-[10px] text-amber-600">
				{{ order.item_brand }}
			</span>
			<span v-if="order.customer_id_type" class="px-1.5 py-0.5 bg-orange-100 dark:bg-orange-900/30 rounded text-[10px] text-orange-600">
				ID ✓
			</span>
		</div>
		...
		<div class="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
			<span class="text-xs text-gray-400">By: {{ order.handled_by_name || 'Unassigned' }}</span>
			<span class="text-sm font-bold text-gray-900 dark:text-white">${{ formatNum(order.estimated_cost || order.total_cost) }}</span>
		</div>
		...
		<!-- Overdue Warning -->
		<div v-if="isOverdue(order.promised_date) && order.status !== 'Delivered'" class="mt-2 text-xs text-red-500 font-medium flex items-center gap-1">
