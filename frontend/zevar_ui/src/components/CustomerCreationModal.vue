	...
	<Teleport to="body">
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="$emit('close')">
			<div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl max-w-md w-full mx-4">
				<div class="p-6">
					<h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Create New Customer</h3>
					<form @submit.prevent="submit" class="space-y-4">
						<!-- Customer Name -->
						<div>
							<label class="block text-sm font-medium mb-1">Customer Name *</label>
							<input v-model="form.customer_name" type="text" placeholder="Full Name" required
						...
						<div>
							...
							<textarea v-model="form.address" rows="2" placeholder="Street address, City, State, ZIP"
						...
						<div>
							...
							<input v-model="form.notes" type="text" placeholder="VIP, preferences, etc."
								class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-800 text-sm" />
						</div>
