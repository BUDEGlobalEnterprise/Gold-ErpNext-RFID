	...
	<form @submit.prevent="$emit('submit')" class="space-y-4">
		...
		<div class="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-3 border border-purple-100 dark:border-purple-800/30">
			<div class="flex items-center justify-between mb-2">
				...
				<button type="button" @click="$emit('add-gemstone')" class="text-xs px-2 py-1 bg-purple-500 text-white rounded hover:bg-purple-600">+ Add Stone</button>
			</div>
			...
			<div v-else class="space-y-2 max-h-40 overflow-y-auto">
				<div v-for="(stone, idx) in form.gemstones" :key="idx" class="bg-white dark:bg-warm-dark-900 rounded p-2 text-xs">
					...
					<div class="grid grid-cols-3 gap-2 mt-1">
						...
						<button type="button" @click="$emit('remove-gemstone', idx)" class="text-red-500 hover:text-red-700">Remove</button>
					</div>
				</div>
			</div>
		</div>
