<template>
	<BaseModal :show="true" max-width="max-w-lg" scrollable @close="$emit('close')">
		...
		<div class="p-6 pt-0">
			<div v-if="mode === 'dispatch'" class="space-y-4">
				<div>
					...
					<select v-model="source" class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-warm-dark-900 text-sm">
						<option value="">Select source store...</option>
						<option v-for="s in storeWarehouses" :key="s.name" :value="s.name">{{ s.warehouse_name || s.name }}</option>
					</select>
				</div>
				<div>
					<label class="block text-sm font-medium mb-1">Destination Store *</label>
					<select v-model="destination" class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-warm-dark-900 text-sm">
						<option value="">Select destination store...</option>
						<option v-for="s in storeWarehouses.filter(w => w.name !== source)" :key="s.name" :value="s.name">{{ s.warehouse_name || s.name }}</option>
					</select>
				</div>
				<div>
					<label class="block text-sm font-medium mb-1">Carrier Reference</label>
					<input v-model="carrierRef" class="w-full px-3 py-2 border rounded-lg bg-white dark:bg-warm-dark-900 text-sm" placeholder="e.g., FedEx tracking #" />
				</div>
				<div>
			...
			<div v-else class="space-y-4">
				<div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 border border-blue-100">
					<p class="text-xs text-blue-700">Scan each serial number to confirm receipt. Any missing pieces will be flagged as variance.</p>
				</div>
				<div>
					<label class="block text-sm font-medium mb-2">Scan Received Pieces</label>
					<ScannerInput @scan="addReceivedSerial" />
					<div class="mt-2 space-y-1 max-h-40 overflow-y-auto">
						<div v-for="(sn, i) in receivedSerials" :key="i" class="flex items-center justify-between text-sm py-1 px-2 bg-green-50 dark:bg-green-900/20 rounded">
							<span class="font-mono">{{ sn }}</span>
		...
		<template #footer>
			...
			<button @click="submit" :disabled="submitting || (mode === 'dispatch' ? serialNos.length === 0 : receivedSerials.length === 0)" class="flex-1 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
				{{ submitting ? 'Processing...' : (mode === 'dispatch' ? `Dispatch ${serialNos.length} Pieces` : `Confirm ${receivedSerials.length} Received`) }}
			</button>
		</template>
	</BaseModal>
</template>
...
const whResource = createResource({
	...
	makeParams: () => ({ doctype: 'Warehouse', filters: { is_group: 1 }, fields: ['name', 'warehouse_name'], limit_page_length: 100 }),
	onSuccess: (data) => { storeWarehouses.value = data || [] },
})
	...
	try {
		if (props.mode === 'dispatch') {
			...
			await call('zevar_core.api.inventory.create_inter_store_transfer', {
				items, source: source.value, destination: destination.value, carrier_ref: carrierRef.value,
			})
			toast({ title: 'Dispatched', message: `${serialNos.value.length} pieces sent`, icon: 'check', intent: 'success' })
		} else {
			await call('zevar_core.api.inventory.receive_inter_store_transfer', {
				transfer_name: props.order.name, scanned_serials: receivedSerials.value,
			})
			toast({ title: 'Received', message: `${receivedSerials.value.length} pieces confirmed`, icon: 'check', intent: 'success' })
		}
		emit('completed')
	} catch (e) {
