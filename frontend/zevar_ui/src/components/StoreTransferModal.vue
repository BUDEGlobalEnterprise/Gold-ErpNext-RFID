			...
			<div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl max-w-md w-full mx-4 p-6">
				...
				<div class="mb-4">
					<p class="text-sm text-gray-600">
						Initiating transfer for <span class="font-bold">{{ order.name }}</span>
					</p>
					<p class="text-xs text-gray-500 mt-1">
						From: {{ order.warehouse || order.receiving_store }}
					</p>
				</div>
				...
				<form @submit.prevent="submitTransfer" class="space-y-4">
					...
					>
						<p class="text-xs text-yellow-700">
							...
							This will create a material transfer and update the repair status. The
							destination store will be notified.
						</p>
					</div>
					...
					<div class="flex gap-3">
						...
						<button
							type="submit"
							:disabled="submitting || !destinationStore"
							class="flex-1 py-2 bg-blue-500 text-white rounded-lg text-sm font-medium hover:bg-blue-600 disabled:opacity-50"
						>
