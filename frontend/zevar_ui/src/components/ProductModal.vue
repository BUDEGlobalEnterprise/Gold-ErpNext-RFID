<template>
	<BaseModal :show="show" max-width="max-w-5xl" :no-max-height="true" @close="close">
		<div class="flex flex-col md:flex-row">
			...
			<!-- Right: Details -->
			<div class="w-full md:w-7/12 p-8 overflow-y-auto bg-[#FAF7F2] dark:bg-warm-dark-800 text-gray-900 dark:text-white">
				...
				<div v-else class="space-y-6">
					<!-- Title & SKU -->
					<div>
						<h2 class="text-2xl font-sans font-bold text-gray-900 dark:text-white leading-tight mb-1">
							{{ details.item_name }}
						</h2>
					...
					<!-- Weight Section -->
					<div class="bg-white dark:bg-warm-dark-900 rounded-xl p-5 border border-[#EFEAE2] dark:border-warm-border/30 shadow-sm">
						<div
							v-if="details.gross_weight"
							class="flex justify-between text-sm mb-2.5"
						>
							<span class="text-gray-600 dark:text-white/70 font-semibold">Gross Weight</span>
					...
					<div class="border border-[#EFEAE2] dark:border-warm-border/30 rounded-xl overflow-hidden shadow-sm bg-white dark:bg-warm-dark-900">
						<div class="bg-[#FAF5EE] dark:bg-warm-dark-850 px-4 py-2.5 border-b border-[#EFEAE2] dark:border-warm-border/20">
							<h3 class="font-bold text-gray-900 dark:text-white text-xs uppercase tracking-wider">
								Product Details
							</h3>
						</div>
						<div class="divide-y divide-[#EFEAE2] dark:divide-warm-border/20">
							<div v-if="details.product_type" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Product Type</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.product_type
								}}</span>
							</div>
							<div v-if="details.jewelry_type" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Jewelry Type</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.jewelry_type
								}}</span>
							</div>
							<div v-if="details.jewelry_subtype" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Sub Type</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.jewelry_subtype
								}}</span>
							</div>
							<div v-if="details.material_color" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Material Color</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.material_color
								}}</span>
							</div>
							<div v-if="details.finish" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Finish</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.finish
								}}</span>
							</div>
							<div v-if="details.plating" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Plating</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.plating
								}}</span>
							</div>
							<div v-if="details.length" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Length of Item</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.length
								}}</span>
							</div>
							<div v-if="details.width" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Width of Item</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.width
								}}</span>
							</div>
							<div v-if="details.size" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Size</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.size
								}}</span>
							</div>
							<div v-if="details.chain_type" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Chain Type</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.chain_type
								}}</span>
							</div>
							<div v-if="details.clasp_type" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Clasp Type</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.clasp_type
								}}</span>
							</div>
							<div v-if="details.gender" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Gender</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.gender
								}}</span>
							</div>
							<div v-if="details.completeness" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Completeness</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
									details.completeness
								}}</span>
							</div>
							<div v-if="details.country_of_origin" class="flex py-2.5 px-4 items-center">
								<span class="w-1/2 text-xs text-gray-500 dark:text-white/60 font-semibold">Country of Origin</span>
								<span class="w-1/2 text-xs text-gray-900 dark:text-white font-bold">{{
					...
					>
						...
						<div class="overflow-x-auto">
							<table class="w-full text-xs">
								<thead class="bg-gray-50/50 dark:bg-warm-dark-850 border-b border-[#EFEAE2] dark:border-warm-border/20">
									<tr class="text-[10px] text-gray-500 dark:text-white/60 uppercase font-semibold">
								...
								<tbody class="divide-y divide-[#EFEAE2] dark:divide-warm-border/20">
									<tr
									...
									>
										...
										<td class="px-4 py-3 text-right font-mono text-gray-900 dark:text-white font-bold">
											{{ gem.carat || '0' }}
										</td>
									</tr>
								</tbody>
					...
					<!-- Price Section -->
					<div class="bg-white dark:bg-warm-dark-900 rounded-xl p-5 border border-[#CBA358]/35 shadow-md">
						<div class="flex items-baseline justify-between mb-4">
							<div>
								<p class="text-xs uppercase tracking-wider text-gray-400 dark:text-white/40 font-bold mb-1">
									Total Price
									<span v-if="details.price_source" class="text-[10px] text-gray-400 font-normal normal-case"
										>({{ details.price_source }})</span
									>
								</p>
								<p class="text-3xl font-black text-[#CBA358] dark:text-[#CBA358] font-mono leading-none">
