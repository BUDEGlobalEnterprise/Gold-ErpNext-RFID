<template>
	<div class="mb-6 p-4 bg-white dark:bg-warm-dark-900 rounded-lg border border-gray-200 dark:border-warm-border">
		...
		<div v-if="!repair.balance_due || repair.balance_due <= 0" class="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
			<div class="w-12 h-12 bg-green-100 dark:bg-green-800 rounded-full flex items-center justify-center mx-auto mb-2 text-green-600 dark:text-green-400">
		...
		<div v-else>
			<div class="flex justify-between items-center mb-4">
				...
				<span class="text-xl font-bold text-gray-900 dark:text-white">${{ formatNum(repair.balance_due) }}</span>
			</div>
			...
			<div v-else-if="paymentUrl" class="space-y-4">
				...
				<a :href="paymentUrl" target="_blank" class="block w-full py-3 bg-[#D4AF37] text-black text-center rounded-lg font-bold hover:bg-[#c9a432] transition">
					Pay Securely
				</a>
			</div>
			...
			<div v-else class="space-y-3">
				...
				<button @click="initiatePayment" class="w-full py-3 bg-gray-900 dark:bg-white text-white dark:text-black rounded-lg font-bold hover:bg-gray-800 dark:hover:bg-gray-200 transition flex items-center justify-center gap-2">
					<span class="material-symbols-outlined">lock</span>
					Pay Online
				</button>
			</div>
			
			<div v-if="error" class="mt-3 p-3 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 rounded-lg text-sm">
				{{ error }}
			</div>
		</div>
	</div>
