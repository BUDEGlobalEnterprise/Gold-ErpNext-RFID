...
export const useAiStore = defineStore('ai', () => {
	...
	function ask(question, contextType = null) {
		if (!isAiEnabled || !question?.trim() || isLoading.value) return
	...
	function searchProducts(query) {
		if (!isAiEnabled || !query?.trim() || isSearching.value) return
	...
	function getRecommendations(customer, occasion = null) {
		if (!isAiEnabled || !customer || isLoading.value) return

		messages.value.push({
			role: 'user',
			content: `Recommend products for customer ${customer}${
				occasion ? ` (${occasion})` : ''
			}`,
			timestamp: new Date(),
		})
