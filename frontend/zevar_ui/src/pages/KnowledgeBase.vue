  ...
  <AppLayout>
    <div class="flex flex-col max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        >
          <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ cat.count }}</div>
          <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ cat.category }}</div>
        </div>
        <div
      ...
      <div class="flex flex-col md:flex-row gap-6 h-[calc(100vh-340px)]">
        ...
        >
          ...
          <div class="space-y-2">
            <div
              ...
              :class="selectedArticle?.name === article.name
              ...
              <div class="flex items-start justify-between gap-2">
                <div class="min-w-0">
                  ...
                  <div class="flex items-center gap-2 mt-1">
                    <span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-200 dark:bg-warm-dark-800 text-gray-600 dark:text-gray-400">
                      {{ article.category }}
                    </span>
                    <span class="text-[10px] px-1.5 py-0.5 rounded"
                      :class="article.visibility === 'Public' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400'"
                    >
                      {{ article.visibility }}
                    </span>
                  </div>
            ...
            <div v-if="!articles.length" class="text-center py-8 text-gray-400 text-sm">
              {{ searchQuery ? 'No articles match your search' : 'No articles in this category' }}
            </div>
          </div>
        </div>
        ...
        >
          <!-- No selection state -->
          <div v-if="!editing && !selectedArticle" class="flex flex-col items-center justify-center h-full text-gray-400">
          ...
          <div v-else-if="!editing && selectedArticle" class="space-y-4">
            <div class="flex items-start justify-between">
              <div>
                ...
                <div class="flex items-center gap-2 mt-2">
                  <span class="text-xs px-2 py-1 rounded bg-gray-100 dark:bg-warm-dark-900 text-gray-600 dark:text-gray-400">
                    {{ selectedArticle.category }}
                  </span>
                  <span class="text-xs px-2 py-1 rounded"
                    :class="selectedArticle.visibility === 'Public' ? 'bg-green-100 dark:bg-green-900/30 text-green-700' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-700'"
                  >
            ...
            <div class="prose dark:prose-invert max-w-none text-sm" v-html="selectedArticle.content"></div>
          </div>
...
<b>bold</b>, <i>italic</i>, <ul><li>lists</li></ul>, <h4>headings</h4>"
            ...
            <p class="text-xs text-gray-400">
              Saved articles are automatically indexed for AI search. The article will appear in AI responses when staff ask related questions.
            </p>
          </div>
        </div>
      </div>
...
const editForm = ref({ title: '', content: '', category: 'FAQ', visibility: 'Internal', tags: '', name: '' })
...
function openEditor(article = null) {
  ...
  if (article) {
    ...
    editForm.value = { title: '', content: '', category: 'FAQ', visibility: 'Internal', tags: '', name: '' }
  }
}
