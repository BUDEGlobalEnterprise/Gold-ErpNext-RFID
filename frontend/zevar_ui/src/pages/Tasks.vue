		...
		<div class="flex flex-col h-full">
			<div class="flex items-center justify-between gap-4 mb-4 flex-shrink-0">
				<div class="flex items-center gap-3">
					...
					<span
						v-if="store.overdueCount"
						class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400"
						>{{ store.overdueCount }} overdue</span
					>
				</div>
				<div class="flex items-center gap-2">
					<div class="flex bg-gray-100 dark:bg-warm-dark-700 rounded-lg p-0.5">
						<button
							...
							:class="
								viewMode === 'kanban'
									? 'bg-white dark:bg-warm-dark-600 text-gray-900 dark:text-white shadow-sm'
									: 'text-gray-500'
							"
						>
							Kanban
			...
			<!-- KPI Cards -->
			<div
				v-if="store.taskStats"
				class="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-4 flex-shrink-0"
			>
				<div class="premium-card !p-3">
					<div class="text-[10px] font-bold text-gray-500 uppercase">Total</div>
					<div class="text-xl font-bold text-gray-900 dark:text-white">
						{{ store.taskStats.total || 0 }}
					</div>
				</div>
				<div class="premium-card !p-3">
					<div class="text-[10px] font-bold text-gray-500 uppercase">To Do</div>
					<div class="text-xl font-bold text-blue-500">
						{{ store.taskStats.by_status?.Todo || 0 }}
					</div>
				</div>
				<div class="premium-card !p-3">
					<div class="text-[10px] font-bold text-gray-500 uppercase">In Progress</div>
					<div class="text-xl font-bold text-amber-500">
						{{ store.taskStats.by_status?.['In Progress'] || 0 }}
					</div>
				</div>
				<div class="premium-card !p-3">
					<div class="text-[10px] font-bold text-gray-500 uppercase">Done</div>
					<div class="text-xl font-bold text-emerald-500">
						{{ store.taskStats.by_status?.Done || 0 }}
					</div>
				</div>
				<div class="premium-card !p-3">
					<div class="text-[10px] font-bold text-gray-500 uppercase">Overdue</div>
					<div class="text-xl font-bold text-red-500">
						{{ store.taskStats.overdue || 0 }}
					</div>
				</div>
			</div>
				...
				<div class="flex gap-3 h-full min-w-[700px]">
					...
					>
						<div class="flex items-center gap-2 mb-3 px-1">
							...
							<span
								class="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-warm-dark-700 text-gray-500"
						...
						<div class="flex-1 overflow-y-auto space-y-2 pr-1">
							<div
								...
								:key="task.id"
								class="premium-card !p-3 cursor-pointer hover:border-[#D4AF37]/50"
								@click="viewTask(task)"
							>
								<div class="text-xs font-bold text-gray-900 dark:text-white mb-1">
									{{ task.title }}
								</div>
								<div
									v-if="task.project_name"
									class="text-[10px] text-gray-500 mb-1.5"
								>
									{{ task.project_name }}
								</div>
							...
							<div
								...
								class="text-center py-8 text-gray-300 dark:text-gray-600 text-xs"
							>
								No tasks
							</div>
						</div>
			...
			<div v-if="viewMode === 'list'" class="flex-1 overflow-y-auto min-h-0">
				<div
					v-if="store.tasksResource.loading && !store.tasks.length"
					...
					<table class="w-full text-sm">
						<thead>
							...
							>
								...
								<th
									class="text-center px-4 py-3 text-[10px] font-bold text-gray-500 uppercase hidden md:table-cell"
								>
									Priority
								</th>
								<th
						...
						<tbody>
							...
							>
								...
								<td
									class="px-4 py-3 text-xs hidden md:table-cell"
									:class="
										t.is_overdue ? 'text-red-500 font-bold' : 'text-gray-500'
									"
								>
									{{ t.due_date || '-' }}
								</td>
			...
			<div v-if="viewMode === 'todos'" class="flex-1 overflow-y-auto min-h-0">
				...
				<div
					v-if="store.todosResource.loading && !store.todos.length"
					class="flex items-center justify-center py-20"
				>
					<div
						class="animate-spin w-8 h-8 border-2 border-[#D4AF37] border-t-transparent rounded-full"
				...
				<div v-else class="space-y-2">
					<div
						...
						:key="todo.id"
						class="premium-card !p-3 flex items-start gap-3"
					>
						<button
							@click="toggleTodo(todo)"
							class="mt-0.5 w-5 h-5 rounded-md border-2 flex items-center justify-center transition shrink-0"
							:class="
								todo.status === 'Closed'
						...
						<div class="flex-1 min-w-0">
							<div
								...
								:class="{ 'line-through opacity-50': todo.status === 'Closed' }"
							>
								{{ todo.description }}
							</div>
							<div class="flex items-center gap-3 mt-1">
								<span v-if="todo.date" class="text-[10px] text-gray-500">{{
									todo.date
								}}</span>
								<span
									v-if="todo.priority"
			...
			>
				<div
					class="premium-card !rounded-2xl w-full max-w-md max-h-[80vh] overflow-y-auto m-4"
				>
					...
					<div class="space-y-3">
						<div>
							...
							><textarea
								...
								class="w-full mt-1 px-3 py-2 bg-gray-50 dark:bg-warm-dark-900 border border-gray-200 dark:border-warm-border rounded-lg text-sm text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-[#D4AF37] resize-none"
							></textarea>
						</div>
						<div class="grid grid-cols-2 gap-3">
							<div>
								...
								><input
									...
									type="date"
									class="w-full mt-1 px-3 py-2 bg-gray-50 dark:bg-warm-dark-900 border border-gray-200 dark:border-warm-border rounded-lg text-sm text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-[#D4AF37]"
								/>
							</div>
							<div>
								<label class="text-[10px] font-bold text-gray-500 uppercase"
					...
					<button
						...
						:disabled="store.createTodoResource.loading"
...
async function handleCreateTodo() {
	if (!todoForm.value.description) {
		toast({ title: 'Description required', icon: 'alert-circle', intent: 'warning' })
		return
	}
