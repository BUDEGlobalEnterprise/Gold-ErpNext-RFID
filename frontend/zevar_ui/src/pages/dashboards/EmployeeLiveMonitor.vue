				...
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
					...
					<div class="premium-card !p-5">
						...
						>
							...
							My Tasks
							<span
								v-if="tasks.pending_count"
								class="ml-auto text-[10px] font-bold bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full"
							>
								{{ tasks.pending_count }}
							</span>
						</h3>
						...
						<template v-else>
							...
							<div v-if="tasks.repairs?.length" class="space-y-2 mb-3">
								...
								>
									...
									></span>
									<span
										class="text-xs text-gray-900 dark:text-white flex-1 truncate"
									...
									<span
										v-if="r.days_overdue > 0"
										class="text-[10px] font-bold text-red-500 shrink-0"
									>
							...
							<div v-if="tasks.layaways?.length" class="space-y-2 mb-3">
								...
								>
									<span
										class="w-2 h-2 rounded-full bg-purple-400 shrink-0"
									></span>
									<span
										class="text-xs text-gray-900 dark:text-white flex-1 truncate"
										>{{ l.customer_name }}</span
									>
									<span class="text-[10px] text-gray-400 shrink-0"
										>${{ fmt(l.outstanding_amount) }}</span
									>
								</div>
							</div>
							<!-- Todos -->
							<div v-if="tasks.todos?.length" class="space-y-2">
								...
								>
									<span class="material-symbols-outlined !text-sm text-gray-300"
										>check_box_outline_blank</span
									>
									<span
										class="text-xs text-gray-700 dark:text-gray-300 flex-1 truncate"
							...
							<p
								v-if="
									!tasks.repairs?.length &&
									!tasks.layaways?.length &&
									!tasks.todos?.length
								"
					...
					<div class="premium-card !p-5">
						...
						<!-- Recent feed -->
						<div
							v-if="store.recent_feed?.length && !loading"
							class="mt-3 space-y-1 max-h-24 overflow-auto"
						>
							<div
							...
							>
								<span class="text-[10px] text-gray-400 shrink-0 w-12">{{
									ev.time
								}}</span>
								...
								<span
									v-if="ev.stream !== 'Sale'"
									class="text-[9px] bg-gray-100 dark:bg-gray-800 text-gray-500 px-1.5 py-0.5 rounded"
									>{{ ev.stream }}</span
								>
							</div>
						</div>
...
async function refresh() {
	...
	try {
		const res = await fetch(
			'/api/method/zevar_core.api.employee_live_monitor.get_employee_dashboard',
			{
