	...
	<AppLayout>
		<div class="flex flex-col h-full">
			<div class="flex items-center gap-3 mb-4 flex-shrink-0 flex-wrap">
				...
				<button @click="refresh" class="ml-auto w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-slate-500 transition-colors" :class="{ 'animate-spin': loading }">
			...
			<div class="flex-1 overflow-auto space-y-4 pr-1">
				<div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
					<KPICard label="Total Items" :value="kpi.total_items" icon="category" color="slate" :loading="loading" />
					<KPICard label="Total Value" :value="'$' + fmt(kpi.total_value)" icon="account_balance" color="emerald" :loading="loading" />
					<KPICard label="Low Stock" :value="kpi.low_stock" icon="warning" color="red" :loading="loading" />
					<KPICard label="In Transit" :value="kpi.in_transit" icon="local_shipping" color="amber" :loading="loading" />
				</div>
				...
				<div class="premium-card !p-3 sm:!p-5">
					...
					<div v-if="loading" class="space-y-2">
						<div v-for="n in 3" :key="n" class="h-4 bg-gray-100 dark:bg-gray-800 rounded-full animate-pulse"></div>
					</div>
					<div v-else-if="storeValues.length" class="space-y-2">
						<div v-for="store in storeValues" :key="store.warehouse" class="flex items-center gap-3">
							<span class="text-xs font-bold text-gray-900 dark:text-white w-20 truncate">{{ store.name || store.warehouse }}</span>
					...
					<p v-else class="text-xs text-gray-400 text-center py-4">No stock data available</p>
				</div>
				...
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
					...
					<div class="premium-card !p-3 sm:!p-5">
						...
						<div v-if="loading" class="space-y-2">
							<div v-for="n in 4" :key="n" class="h-4 bg-gray-100 dark:bg-gray-800 rounded animate-pulse"></div>
						</div>
					...
					<div class="premium-card !p-3 sm:!p-5">
						...
						<div v-else-if="shrinkageTrend.length" class="h-32 sm:h-40 overflow-x-auto">
							<div class="flex items-end gap-2 min-w-[300px] h-full">
								<div v-for="(m, i) in shrinkageTrend" :key="i" class="flex-1 flex flex-col items-center gap-1 group cursor-default">
									<span class="text-[8px] text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity">${{ fmt(m.value) }}</span>
									<div class="w-full rounded-t bg-red-500/60 dark:bg-red-400/40 min-h-[2px] hover:bg-red-500 transition-colors" :style="{ height: m.height + '%' }"></div>
									<span class="text-[8px] text-gray-400">{{ m.label }}</span>
								</div>
							</div>
						</div>
						<p v-else class="text-xs text-gray-400 text-center py-8">No shrinkage data</p>
					</div>
				</div>
			</div>
		</div>
...
async function refresh() {
	...
	error.value = null
	try {
		const res = await fetch('/api/method/zevar_core.api.inventory_dashboard.get_dashboard_data', {
