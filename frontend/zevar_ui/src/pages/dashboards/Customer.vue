	...
	<AppLayout>
		<div class="flex flex-col h-full">
			<div class="flex items-center gap-3 mb-4 flex-shrink-0 flex-wrap">
				...
				<div
					class="w-10 h-10 rounded-xl flex items-center justify-center bg-purple-500/10"
				>
				...
				<div>
					...
					<p class="text-[10px] text-gray-400">
						New vs returning, LTV, layaway cohort, repair NPS
					</p>
				</div>
				<button
					@click="refresh"
					class="ml-auto w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-purple-500 transition-colors"
			...
			<div class="flex-1 overflow-auto space-y-4 pr-1">
				...
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
					...
					<div class="premium-card !p-3 sm:!p-5">
						...
						<div v-if="loading" class="flex gap-3">
							<div
								class="flex-1 h-20 bg-gray-100 dark:bg-gray-800 rounded-lg animate-pulse"
							></div>
							<div
								class="flex-1 h-20 bg-gray-100 dark:bg-gray-800 rounded-lg animate-pulse"
							></div>
						...
						<div v-else class="flex gap-3">
							<div
								class="flex-1 bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4 text-center"
							>
								<p
									class="text-2xl font-black text-purple-700 dark:text-purple-300"
								>
							...
							<div
								class="flex-1 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 text-center"
							>
					...
					<div class="premium-card !p-3 sm:!p-5">
						...
						<div v-else-if="topCustomers.length" class="space-y-2">
							...
							>
								...
								>
								<span
									class="text-xs font-black text-purple-600 dark:text-purple-400"
									>${{ fmt(c.total) }}</span
								>
							</div>
						</div>
				...
				<div class="premium-card !p-3 sm:!p-5">
					<h3 class="text-sm font-bold text-gray-900 dark:text-white mb-3">
						Layaway Cohort Retention
					</h3>
					<p class="text-xs text-gray-500 dark:text-gray-400">
						Showing active layaway completion rate by month of origination.
					</p>
					...
					<div v-else-if="cohortData.length" class="mt-3 h-32 sm:h-40 overflow-x-auto">
						<div class="flex items-end gap-2 min-w-[300px] h-full">
							...
							>
								...
								>
								<div
									class="w-full rounded-t bg-purple-500/60 dark:bg-purple-400/40 min-h-[2px] hover:bg-purple-500 transition-colors"
					...
					<p v-else class="text-xs text-gray-400 text-center py-8 mt-2">
						No layaway cohort data
					</p>
				</div>
			</div>
		</div>
...
async function refresh() {
	...
	try {
		const res = await fetch(
			'/api/method/zevar_core.api.customer_dashboard.get_dashboard_data',
			{
