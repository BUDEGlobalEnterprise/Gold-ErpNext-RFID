				...
				<div v-else class="space-y-4">
					<div class="premium-card !p-6">
						<div class="flex items-center gap-4">
							<div
								class="w-16 h-16 rounded-full bg-[#D4AF37]/20 flex items-center justify-center flex-shrink-0"
							>
							...
							<div>
								<div class="text-lg font-bold text-gray-900 dark:text-white">
									{{ hr.employee.employee_name }}
								</div>
								<div class="text-sm text-gray-500 dark:text-gray-400">
									{{ hr.employee.designation || 'No designation' }}
								</div>
								<div class="text-xs text-gray-400">
									{{ hr.employee.department || '' }} &middot;
					...
					<div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
						<div
							class="premium-card !p-4 cursor-pointer hover:ring-1 hover:ring-[#D4AF37]/30"
							@click="$router.push('/time-clock')"
						>
							<div
							...
							<div
								v-if="hr.todayStatus?.total_hours_today"
								class="text-lg font-bold text-[#D4AF37]"
							>
								{{ hr.todayStatus.total_hours_today }}h
							</div>
						</div>
						<div
							class="premium-card !p-4 cursor-pointer hover:ring-1 hover:ring-[#D4AF37]/30"
						...
						>
							...
							<div class="text-xs font-bold text-gray-600 dark:text-gray-300">
								{{ totalLeaveRemaining }} days remaining
							</div>
						</div>
					...
					<div class="premium-card !p-4">
						...
						<div v-else class="space-y-2">
							<div
								v-for="slip in hr.salarySlips.slice(0, 5)"
								:key="slip.id"
								class="flex items-center justify-between py-2 border-b border-gray-100 dark:border-warm-dark-700 last:border-0"
							>
								<div>
					...
					<div class="premium-card !p-4">
						...
						<div v-else class="space-y-2">
							<div
								v-for="claim in hr.expenseClaims.slice(0, 5)"
								:key="claim.name"
								class="flex items-center justify-between py-2 border-b border-gray-100 dark:border-warm-dark-700 last:border-0"
							>
								<div>
								...
								<div class="text-right">
									...
									<span
										...
										:class="expenseStatusBadge(claim.status)"
									>
										{{ claim.status }}
									</span>
								</div>
...
const totalLeaveRemaining = computed(() =>
	hr.leaveBalances.reduce((s, b) => s + (b.remaining || 0), 0)
)
