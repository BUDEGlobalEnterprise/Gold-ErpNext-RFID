<template>
	<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
			...
			<div class="max-w-6xl mx-auto px-4 py-4">
				<div class="flex items-center justify-between">
					<div class="flex items-center gap-3">
						...
						<span class="text-gray-600 dark:text-gray-400 hidden sm:inline">Customer Portal</span>
					</div>
					<div v-if="authenticated" class="flex items-center gap-3">
						<span class="text-sm text-gray-600 dark:text-gray-400">{{ customer?.customer_name }}</span>
						<button @click="logout" class="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400">
							Sign Out
						</button>
					</div>
				</div>
			...
			<div v-if="!authenticated" class="max-w-md mx-auto">
				<div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-8">
					<div class="text-center mb-8">
						...
						<h2 class="text-2xl font-bold text-gray-900 dark:text-white">Welcome to the Customer Portal</h2>
						<p class="text-gray-600 dark:text-gray-400 mt-2">Enter your phone number or email to access your repairs</p>
					</div>
					...
					<div v-if="authStep === 'lookup'">
						<form @submit.prevent="lookupCustomer">
							<div class="space-y-4">
								<div>
									<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Phone Number or Email</label>
									<input v-model="lookupIdentifier" type="text"
								...
								<button type="submit" :disabled="lookingUp || !lookupIdentifier"
							...
							<div class="space-y-4">
								<div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-100 dark:border-blue-800">
									<p class="text-sm text-blue-800 dark:text-blue-300 text-center">
										We've sent a verification code to your phone/email. Enter it below to continue.
									</p>
								</div>
								<div>
								...
								<button type="submit" :disabled="verifying || !verificationCode"
									class="w-full py-3 bg-[#D4AF37] text-black rounded-lg font-bold hover:bg-[#c9a432] disabled:opacity-50 disabled:cursor-not-allowed">
									{{ verifying ? 'Verifying...' : 'Verify & Continue' }}
								</button>
								<button type="button" @click="authStep = 'lookup'; verificationCode = ''"
					...
					<!-- Error -->
					<div v-if="authError" class="mt-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-100 dark:border-red-800">
						<p class="text-sm text-red-800 dark:text-red-300 text-center">{{ authError }}</p>
					</div>
				</div>
			</div>
				...
				<div class="bg-white dark:bg-gray-900 rounded-t-2xl shadow-lg px-4 pt-4">
					<div class="flex gap-2">
						<button v-for="tab in tabs" :key="tab.key" @click="activeTab = tab.key"
							...
							{{ tab.label }}
							<span v-if="tab.badge" class="ml-1 px-2 py-0.5 text-xs rounded-full" :class="tab.badgeClass">{{ tab.badge }}</span>
						</button>
					</div>
				</div>
				...
				<div class="bg-white dark:bg-gray-900 rounded-b-2xl shadow-lg p-6">
					...
					<div v-if="activeTab === 'repairs'">
						<div v-if="loadingRepairs" class="text-center py-8">
							<div class="animate-spin w-8 h-8 border-2 border-[#D4AF37] border-t-transparent rounded-full mx-auto"></div>
						...
						<div v-else-if="repairs.length === 0" class="text-center py-8">
							...
							<h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2">No Active Repairs</h3>
							<p class="text-gray-600 dark:text-gray-400">You don't have any active repairs at the moment.</p>
						</div>

						<div v-else class="space-y-4">
							<div v-for="repair in repairs" :key="repair.name"
								@click="selectedRepair = repair; showRepairDetail = true"
								...
								<div class="flex items-start justify-between">
									<div>
										<p class="font-bold text-gray-900 dark:text-white">{{ repair.name }}</p>
										<p class="text-sm text-gray-500">{{ repair.repair_type_name || repair.repair_type }}</p>
									</div>
									<span :class="getStatusBadgeClass(repair.status)" class="px-3 py-1 rounded-full text-xs font-medium">
										{{ repair.status }}
									</span>
								</div>
								<div class="mt-3 grid grid-cols-2 gap-2 text-sm">
									<div>
										...
										<span class="ml-1 font-medium">{{ formatDate(repair.promised_date) }}</span>
									</div>
									<div>
										<span class="text-gray-500">Balance:</span>
										<span class="ml-1 font-medium">${{ formatNum(repair.balance_due) }}</span>
									</div>
								</div>
							</div>
						</div>
					...
					<div v-if="activeTab === 'history'">
						...
						<div v-else class="space-y-3">
							<div v-for="item in history" :key="item.repair_number"
								class="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
								<div class="flex items-center justify-between">
									<div>
										<p class="font-medium text-gray-900 dark:text-white">{{ item.repair_number }}</p>
										<p class="text-sm text-gray-500">{{ item.repair_type_name || 'Repair' }}</p>
									</div>
									<div class="text-right">
										<p class="font-medium">${{ formatNum(item.total_cost) }}</p>
										<p class="text-sm text-gray-500">{{ formatDate(item.delivered_date) }}</p>
									</div>
								</div>
								<div v-if="item.warranty?.has_warranty" class="mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
									<p class="text-xs" :class="item.warranty.is_valid ? 'text-green-600' : 'text-red-600'">
										<span class="font-medium">Warranty:</span>
										{{ item.warranty.is_valid ? `Valid for ${item.warranty.days_remaining} days` : 'Expired' }}
									</p>
								</div>
							</div>
						</div>
					...
					<div v-if="activeTab === 'messages'">
						<div class="text-center py-8">
							...
							<p class="text-gray-600 dark:text-gray-400 mb-6">Have questions about your repair? Send us a message.</p>

							<form @submit.prevent="sendMessage" class="max-w-md mx-auto text-left">
								<div class="space-y-4">
									<div>
										<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select Repair</label>
										<select v-model="messageRepairId" class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800">
											<option value="">General Inquiry</option>
											<option v-for="r in repairs.filter(r => r.status !== 'Delivered')" :key="r.name" :value="r.name">
									...
									<div>
										...
										<textarea v-model="messageText" rows="4" placeholder="Type your message..."
											class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800"></textarea>
									</div>
									<button type="submit" :disabled="sendingMessage || !messageText"
		...
		<!-- Repair Detail Modal -->
		<div v-if="showRepairDetail && selectedRepair" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" @click.self="showRepairDetail = false">
			<div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
				<div class="p-6">
					<div class="flex items-start justify-between mb-6">
						<div>
							<h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ selectedRepair.name }}</h2>
							<p class="text-gray-500">{{ selectedRepair.repair_type_name }}</p>
						</div>
						<button @click="showRepairDetail = false" class="text-gray-400 hover:text-gray-600">
					...
					<!-- Status -->
					<div class="mb-6 p-4 rounded-lg" :class="getStatusHeaderClass(selectedRepair.status)">
						<div class="flex items-center justify-between">
					...
					<div class="grid grid-cols-2 gap-4 mb-6">
						...
						<div>
							...
							<p class="font-medium">{{ formatDate(selectedRepair.received_date) }}</p>
						</div>
						<div>
							<p class="text-xs text-gray-500 uppercase">Warehouse</p>
							<p class="font-medium">{{ selectedRepair.warehouse_name || selectedRepair.warehouse || 'N/A' }}</p>
						</div>
					</div>

					<!-- Financial -->
					<div class="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
						<h3 class="font-bold text-gray-900 dark:text-white mb-3">Financial Summary</h3>
						<div class="space-y-2 text-sm">
							<div class="flex justify-between">
								<span class="text-gray-500">Estimated Cost:</span>
								<span class="font-medium">${{ formatNum(selectedRepair.estimated_cost) }}</span>
							</div>
							<div class="flex justify-between">
								<span class="text-gray-500">Total Cost:</span>
								<span class="font-medium">${{ formatNum(selectedRepair.total_cost) }}</span>
							</div>
							<div class="flex justify-between">
								<span class="text-gray-500">Deposit Paid:</span>
								<span class="font-medium text-green-600">-${{ formatNum(selectedRepair.deposit_amount) }}</span>
							</div>
							<div class="flex justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
								<span class="font-bold">Balance Due:</span>
								<span class="font-bold" :class="selectedRepair.balance_due > 0 ? 'text-red-600' : 'text-green-600'">
					...
					<!-- Estimate Approval -->
					<div v-if="selectedRepair.estimate_status === 'Sent'" class="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-100 dark:border-yellow-800">
						<h3 class="font-bold text-yellow-800 dark:text-yellow-300 mb-2">Estimate Pending Approval</h3>
						<p class="text-sm text-yellow-700 dark:text-yellow-400 mb-4">
							Please review and approve the estimate for us to begin work.
						</p>
						<div class="flex gap-2">
							<button @click="approveEstimate" :disabled="approving" class="flex-1 py-2 bg-green-500 text-white rounded-lg font-medium hover:bg-green-600 disabled:opacity-50">
								{{ approving ? 'Approving...' : 'Approve' }}
							</button>
							<button @click="rejecting = true" :disabled="approving" class="flex-1 py-2 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 rounded-lg font-medium hover:bg-red-200 dark:hover:bg-red-800">
								Reject
							</button>
						</div>
					</div>
					...
					<div v-if="selectedRepair.status !== 'Delivered'" class="mb-6">
						<h3 class="font-bold text-gray-900 dark:text-white mb-3">Upload Reference Photo</h3>
						<div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center">
							<input ref="photoInput" type="file" accept="image/*" class="hidden" @change="handlePhotoUpload">
							<button @click="$refs.photoInput?.click()" class="px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-lg font-medium hover:bg-gray-200 dark:hover:bg-gray-700">
								Choose Photo
							</button>
							<p class="text-sm text-gray-500 mt-2">Upload reference photos to help with your repair</p>
						</div>
					</div>
				</div>
			</div>
		...
		<div v-if="rejecting" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
			<div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-md w-full p-6">
				<h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Reject Estimate</h3>
				<p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Please let us know why you're rejecting this estimate so we can help you better.</p>
				<textarea v-model="rejectReason" rows="3" placeholder="Reason for rejection..."
					class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 mb-4"></textarea>
				<div class="flex gap-2">
					<button @click="rejecting = false; rejectReason = ''" class="flex-1 py-2 bg-gray-100 dark:bg-gray-800 rounded-lg font-medium">Cancel</button>
					<button @click="rejectEstimate" :disabled="!rejectReason || rejecting" class="flex-1 py-2 bg-red-500 text-white rounded-lg font-medium hover:bg-red-600 disabled:opacity-50">
...
// Computed
const tabs = computed(() => [
	{ key: 'repairs', label: 'Active Repairs', badge: repairs.value.length || null, badgeClass: 'bg-gray-200 text-gray-700' },
...
async function loadRepairs() {
	...
	try {
		...
		if (result.success) {
			customer.value = result.customer
			repairs.value = result.repairs.filter(r => r.status !== 'Delivered' && r.status !== 'Cancelled')
		}
	} catch (e) {
...
async function approveEstimate() {
	...
	try {
		const result = await call('zevar_core.api.repair_customer_portal.customer_approve_estimate', {
			auth_token: authToken.value,
...
async function rejectEstimate() {
	...
	try {
		const result = await call('zevar_core.api.repair_customer_portal.customer_reject_estimate', {
			auth_token: authToken.value,
			repair_order: selectedRepair.value.name,
			customer_name: customer.value?.customer_name || 'Customer',
...
function handlePhotoUpload(event) {
	...
	reader.onload = async () => {
		try {
			const result = await call('zevar_core.api.repair_customer_portal.upload_reference_photo', {
				auth_token: authToken.value,
				repair_order: selectedRepair.value.name,
