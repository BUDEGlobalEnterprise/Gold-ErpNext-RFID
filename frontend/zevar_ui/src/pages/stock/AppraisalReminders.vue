<template>
	<AppLayout>
		<div class="flex flex-col h-full">
				...
				<div class="flex items-center gap-2">
					...
					>
						<svg
							...
							:class="{ 'animate-spin': store.expiringAppraisalsResource.loading }"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
			...
			<!-- Summary -->
			<div
				v-if="store.expiringAppraisals.length"
				class="mb-4 px-4 py-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg"
			>
				<span class="text-xs font-semibold text-yellow-800 dark:text-yellow-300">
					{{ store.expiringAppraisals.length }} appraisals expiring within
					{{ daysWindow }} days
				</span>
			</div>
			...
			<div class="flex-1 overflow-auto space-y-2">
				<div
					v-for="a in store.expiringAppraisals"
					:key="a.name"
						...
						<div class="flex items-center gap-3">
							<div>
								...
								<div class="text-xs text-gray-500">
									<span v-if="a.serial_no"
										>Serial: {{ a.serial_no }} &middot;
									</span>
			...
			<div
				v-if="
					!store.expiringAppraisals.length && !store.expiringAppraisalsResource.loading
				"
