	...
	<AppLayout>
		<div class="flex flex-col h-full">
			...
			<div class="flex-1 overflow-y-auto min-h-0">
				<div v-if="stock.supplierOrdersResource.loading && !stock.supplierOrders.length" class="flex items-center justify-center py-20">
					<div class="animate-spin w-8 h-8 border-2 border-[#D4AF37] border-t-transparent rounded-full"></div>
				</div>

				<div v-else-if="filteredOrders.length === 0" class="flex flex-col items-center justify-center py-20 text-gray-400">
					...
					<table class="w-full text-sm">
						<thead>
							<tr class="bg-gray-50 dark:bg-warm-dark-700 border-b border-gray-200 dark:border-warm-border/50">
								<th class="text-left px-4 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Order</th>
								<th class="text-left px-4 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider hidden sm:table-cell">Supplier</th>
								<th class="text-left px-4 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider hidden md:table-cell">Date</th>
								<th class="text-left px-4 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Delivery</th>
								...
								<th class="text-center px-4 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider hidden md:table-cell">Received</th>
							</tr>
						</thead>
						<tbody>
							<tr
							...
							>
								<td class="px-4 py-3">
									<div class="font-bold text-gray-900 dark:text-white text-xs">{{ order.name }}</div>
								</td>
								...
								<td class="px-4 py-3 text-center hidden md:table-cell">
									<div class="w-full bg-gray-200 dark:bg-warm-dark-900 rounded-full h-1.5">
										<div
											class="bg-emerald-500 h-1.5 rounded-full transition-all"
											:style="{ width: (order.per_received || 0) + '%' }"
										></div>
									</div>
									<span class="text-[9px] text-gray-500 mt-0.5">{{ Math.round(order.per_received || 0) }}%</span>
								</td>
							</tr>
						</tbody>
					</table>
			...
			<div v-if="selectedOrder" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="selectedOrder = null">
				<div class="premium-card !rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto m-4">
					<div class="flex items-center justify-between mb-4">
						<h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ selectedOrder.name }}</h3>
						<button @click="selectedOrder = null" class="p-1 hover:bg-gray-100 dark:hover:bg-warm-dark-700 rounded-lg">
					...
					<div class="grid grid-cols-2 gap-3 mb-4">
						<div><span class="text-[10px] text-gray-500 uppercase">Supplier</span><p class="text-sm font-bold text-gray-900 dark:text-white">{{ selectedOrder.supplier_name }}</p></div>
						<div><span class="text-[10px] text-gray-500 uppercase">Status</span><p class="text-sm font-bold" :class="statusClass(selectedOrder.status)">{{ selectedOrder.status }}</p></div>
						<div><span class="text-[10px] text-gray-500 uppercase">Date</span><p class="text-sm text-gray-700 dark:text-gray-300">{{ formatDate(selectedOrder.transaction_date) }}</p></div>
						<div><span class="text-[10px] text-gray-500 uppercase">Total</span><p class="text-sm font-bold text-[#D4AF37]">{{ formatCurrency(selectedOrder.grand_total) }}</p></div>
					</div>
					<div v-if="selectedOrder.items" class="border-t border-gray-200 dark:border-warm-border/50 pt-4">
						<h4 class="text-xs font-bold text-gray-500 uppercase mb-2">Items</h4>
						<div v-for="item in selectedOrder.items" :key="item.item_code" class="flex items-center justify-between py-2 border-b border-gray-100 dark:border-warm-border/30 last:border-0">
							<div>
								<div class="text-xs font-bold text-gray-900 dark:text-white">{{ item.item_name }}</div>
								<div class="text-[10px] text-gray-500">{{ item.item_code }}</div>
							</div>
							<div class="text-right">
								<div class="text-xs font-bold text-gray-900 dark:text-white">{{ item.qty }} × {{ formatCurrency(item.rate) }}</div>
								<div class="text-[10px] text-gray-500">Rcvd: {{ item.received_qty || 0 }}</div>
							</div>
						</div>
					</div>
					<div v-if="selectedOrder.docstatus === 0" class="flex gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-warm-border/50">
						<button @click="handleSubmit" class="flex-1 py-2 bg-emerald-600 text-white rounded-lg text-xs font-bold hover:bg-emerald-700 transition">Submit</button>
						<button @click="handleCancel" class="flex-1 py-2 bg-red-50 text-red-600 border border-red-200 rounded-lg text-xs font-bold hover:bg-red-100 transition">Cancel</button>
					</div>
				</div>
			</div>
			...
			<div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="showCreateModal = false">
				<div class="premium-card !rounded-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto m-4">
					<div class="flex items-center justify-between mb-4">
						<h3 class="text-lg font-bold text-gray-900 dark:text-white">New Purchase Order</h3>
						<button @click="showCreateModal = false" class="p-1 hover:bg-gray-100 dark:hover:bg-warm-dark-700 rounded-lg">
							<svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
							</svg>
						</button>
					...
					<div class="space-y-3">
						...
						<div>
							<label class="text-[10px] font-bold text-gray-500 uppercase">Expected Delivery</label>
							<input
						...
						<div>
							<div class="flex items-center justify-between mb-2">
								...
								<button @click="addNewOrderItem" class="text-[10px] font-bold text-[#D4AF37] hover:underline">+ Add Item</button>
							</div>
							<div v-for="(item, idx) in newOrder.items" :key="idx" class="flex gap-2 mb-2">
								<input v-model="item.item_code" placeholder="Item Code" class="flex-1 px-2 py-1.5 bg-gray-50 dark:bg-warm-dark-900 border border-gray-200 dark:border-warm-border rounded text-xs text-gray-900 dark:text-white outline-none" />
								<input v-model.number="item.qty" type="number" min="1" placeholder="Qty" class="w-16 px-2 py-1.5 bg-gray-50 dark:bg-warm-dark-900 border border-gray-200 dark:border-warm-border rounded text-xs text-gray-900 dark:text-white outline-none" />
								<input v-model.number="item.rate" type="number" min="0" placeholder="Rate" class="w-20 px-2 py-1.5 bg-gray-50 dark:bg-warm-dark-900 border border-gray-200 dark:border-warm-border rounded text-xs text-gray-900 dark:text-white outline-none" />
								<button @click="newOrder.items.splice(idx, 1)" class="p-1 text-red-400 hover:text-red-600">
...
const newOrder = ref({ supplier: '', schedule_date: '', items: [{ item_code: '', qty: 1, rate: 0 }] })
...
const pendingCount = computed(() => stock.supplierOrders.filter((o) => ['Draft', 'To Receive and Bill'].includes(o.status)).length)
const completedCount = computed(() => stock.supplierOrders.filter((o) => o.status === 'Completed').length)
const totalValue = computed(() => stock.supplierOrders.reduce((sum, o) => sum + (o.grand_total || 0), 0))
...
async function handleCreate() {
	...
	await stock.createPO(newOrder.value.supplier, JSON.stringify(validItems), null, newOrder.value.schedule_date)
	toast({ title: 'Purchase order created', icon: 'check', intent: 'success' })
	showCreateModal.value = false
	newOrder.value = { supplier: '', schedule_date: '', items: [{ item_code: '', qty: 1, rate: 0 }] }
	loadData()
}
...
function statusClass(status) {
	const map = {
		...
		'To Receive and Bill': 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400',
