	...
	<AppLayout>
		<div class="flex flex-col h-full">
			<div class="flex items-center justify-between gap-4 mb-4 flex-shrink-0">
				...
				<div class="flex items-center gap-2">
					<select
						v-model="memoClass"
						@change="loadData"
						class="px-3 py-1.5 bg-gray-50 dark:bg-warm-dark-900 border border-gray-200 dark:border-warm-border rounded-lg text-xs text-gray-900 dark:text-white"
					>
					...
					>
						<svg
							...
							:class="{ 'animate-spin': store.memoAgingResource.loading }"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
			...
			<div class="grid grid-cols-4 gap-3 mb-6">
				<div
					v-for="(bucket, key) in buckets"
					:key="key"
					class="bg-white dark:bg-warm-dark-800 border rounded-xl p-4"
					:class="bucketBorderClass(key)"
				>
					<div class="text-xs text-gray-500 mb-1">{{ key }} days</div>
					<div class="text-2xl font-bold" :class="bucketTextClass(key)">
						{{ bucket.length }}
					</div>
			...
			<div v-for="(bucket, key) in buckets" :key="key" class="mb-4">
				<div v-if="bucket.length" class="mb-3">
					...
					<div class="space-y-2">
						<div
							v-for="memo in bucket"
							:key="memo.memo_contract"
						...
						>
							...
							<div class="flex items-center gap-4">
								...
								<button
									@click="openMemoActions(memo)"
									class="px-2 py-1 text-[10px] font-bold text-[#D4AF37] border border-[#D4AF37] rounded hover:bg-[#D4AF37]/10"
								>
									Actions
								</button>
							</div>
			...
			<div
				v-if="!hasData && !store.memoAgingResource.loading"
			...
			<BaseModal
				...
				:title="`Memo ${activeMemo?.memo_contract}`"
			>
				<div class="p-4 space-y-3">
					<div class="text-xs text-gray-500">
						{{ activeMemo?.memo_class }} memo for
						{{
							activeMemo?.memo_class === 'Customer'
								? activeMemo?.customer
								: activeMemo?.vendor
						}}
						&mdash; {{ activeMemo?.days_out }} days out,
						{{ activeMemo?.open_items }} open items
					</div>
					<div class="space-y-2">
						<div>
					...
					<div class="flex gap-2 pt-2">
						<button
							@click="doMarkSold"
							class="flex-1 px-3 py-2 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-700"
						>
							Mark Sold
						</button>
						<button
							@click="doMarkReturned"
							class="flex-1 px-3 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold hover:bg-blue-700"
						>
							Mark Returned
						</button>
					</div>
...
const buckets = computed(
	() => store.memoAging?.buckets || { '0-7': [], '8-14': [], '15-30': [], '30+': [] }
)
...
async function doMarkSold() {
	...
	try {
		await store.markMemoSold(
			activeMemo.value.memo_contract,
			actionItemCode.value,
			actionSerial.value || undefined
		)
