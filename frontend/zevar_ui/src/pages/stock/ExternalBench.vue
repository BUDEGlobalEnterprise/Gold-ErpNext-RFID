<template>
	<AppLayout>
		<div class="flex flex-col h-full">
				...
				<div class="flex items-center gap-2">
					...
					>
						<svg
							...
							:class="{ 'animate-spin': store.benchStatusResource.loading }"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
					...
					<div class="flex items-center justify-between">
						<div>
							...
							<div class="text-xs text-gray-500">
								Customer: {{ repair.customer }} &middot; Status:
								{{ repair.status }}
							</div>
			...
			<div
				v-else-if="selectedVendor && !store.benchStatusResource.loading"
			...
			<BaseModal v-if="showReceive" @close="showReceive = false" title="Receive from Bench">
				<div class="p-4 space-y-3">
					...
					<div class="flex justify-end gap-2">
						...
						<button
							@click="doReceive"
							:disabled="store.receiveBenchResource.loading"
							class="px-4 py-2 bg-green-600 text-white rounded-lg text-xs font-bold disabled:opacity-50"
						>
							Confirm Received
						</button>
					</div>
...
onMounted(async () => {
	...
	try {
		...
		const res = createResource({
			url: 'zevar_core.api.inventory_v2.list_external_bench_vendors',
		})
