	...
	<AppLayout>
		<div class="flex flex-col h-full">
				...
				<div class="flex items-center gap-2">
					...
					>
						<svg
							...
							:class="{ 'animate-spin': store.quotesResource.loading }"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
			...
			<div class="flex-1 overflow-y-auto min-h-0">
				<div
					v-if="store.quotesResource.loading && !store.quotes.length"
			...
			>
				...
				>
					...
					<div class="grid grid-cols-2 gap-3 mb-4">
						<div>
							<span class="text-[10px] text-gray-500 uppercase">Customer</span>
						...
						<div>
							...
							<p class="text-sm font-bold text-[#D4AF37]">
								{{ formatCurrency(selected.grand_total) }}
							</p>
						</div>
					</div>
					<div
					...
					<!-- Actions -->
					<div
						class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-warm-border/50"
					>
						<button
							v-if="selected.docstatus === 0"
							@click="handleSubmit"
							class="flex-1 min-w-[120px] py-2 bg-emerald-600 text-white rounded-lg text-xs font-bold hover:bg-emerald-700 transition"
						>
							Submit
						</button>
						<button
							v-if="selected.docstatus === 1 && selected.status !== 'Ordered'"
							@click="handleConvertOrder"
							:disabled="store.convertToOrderResource.loading"
							class="flex-1 min-w-[120px] py-2 bg-blue-600 text-white rounded-lg text-xs font-bold hover:bg-blue-700 transition disabled:opacity-50"
						>
							{{ store.convertToOrderResource.loading ? '...' : '→ Sales Order' }}
						</button>
						<button
							v-if="selected.docstatus === 1 && selected.status !== 'Ordered'"
							@click="handleConvertInvoice"
							:disabled="store.convertToInvoiceResource.loading"
							class="flex-1 min-w-[120px] py-2 bg-purple-600 text-white rounded-lg text-xs font-bold hover:bg-purple-700 transition disabled:opacity-50"
						>
							{{ store.convertToInvoiceResource.loading ? '...' : '→ Invoice' }}
						</button>
						<button
							v-if="selected.docstatus === 1 && selected.status === 'Open'"
							@click="handleMarkLost"
							class="flex-1 min-w-[120px] py-2 bg-red-50 text-red-600 border border-red-200 rounded-lg text-xs font-bold hover:bg-red-100 transition"
						>
							Mark Lost
						</button>
					</div>
			...
			>
				...
				>
						...
						<div>
							<div class="flex items-center justify-between mb-2">
								<label class="text-[10px] font-bold text-gray-500 uppercase"
									>Items</label
								><button
									@click="form.items.push({ item_code: '', qty: 1, rate: 0 })"
									class="text-[10px] font-bold text-[#D4AF37]"
								>
									+ Add
								</button>
							</div>
							...
							>
								<input
									...
									class="flex-1 px-2 py-1.5 bg-gray-50 dark:bg-warm-dark-900 border border-gray-200 dark:border-warm-border rounded text-xs outline-none text-gray-900 dark:text-white"
								/>
								<input
									v-model.number="item.qty"
									type="number"
									min="1"
									class="w-16 px-2 py-1.5 bg-gray-50 dark:bg-warm-dark-900 border border-gray-200 dark:border-warm-border rounded text-xs outline-none text-gray-900 dark:text-white"
								/>
								<input
									...
									min="0"
									placeholder="Rate"
									class="w-20 px-2 py-1.5 bg-gray-50 dark:bg-warm-dark-900 border border-gray-200 dark:border-warm-border rounded text-xs outline-none text-gray-900 dark:text-white"
								/>
								<button
					...
					<button
						@click="handleCreate"
						:disabled="store.createQuoteResource.loading"
...
const openCount = computed(
	() => store.quotes.filter((q) => ['Draft', 'Open'].includes(q.status)).length
)
...
function viewQuote(q) {
	store.loadQuoteDetail(q.name).then(() => {
		selected.value = store.currentQuote
	})
}
...
async function handleCreate() {
	if (!form.value.customer) {
		toast({ title: 'Customer required', icon: 'alert-circle', intent: 'warning' })
		return
	}
	const validItems = form.value.items.filter((i) => i.item_code)
	if (!validItems.length) {
		toast({ title: 'Add items', icon: 'alert-circle', intent: 'warning' })
		return
	}
