		...
		<div class="flex flex-col h-full">
			...
			<div class="flex-1 overflow-y-auto min-h-0">
				...
				<div v-else class="space-y-2">
					<div class="premium-card !p-3 flex items-center gap-3 mb-1">
						<input
							type="checkbox"
							:checked="allSelected"
							@change="toggleAll"
					...
					<div
						...
						:class="{
							'ring-1 ring-[#D4AF37]/50': selectedInvoices.includes(inv.name),
						}"
					...
					>
						...
						<div class="flex-1 min-w-0">
							...
							<div class="text-[11px] text-gray-500 dark:text-gray-400">
								{{ inv.party || '' }} &middot; {{ inv.posting_date }}
							</div>
						</div>
			...
			<div
				...
				@click.self="exportResult = null"
			>
				<div class="premium-card !p-6 w-full max-w-2xl mx-4 max-h-[80vh] overflow-y-auto">
					<h3 class="premium-title !text-lg mb-4">
						Export Result ({{ exportResult.count }} invoices)
					</h3>
					<pre
						class="bg-gray-100 dark:bg-warm-dark-800 p-4 rounded-lg text-xs overflow-auto max-h-[60vh] text-gray-700 dark:text-gray-300"
						>{{ JSON.stringify(exportResult.export_data, null, 2) }}</pre
					>
...
const allSelected = computed(
	() =>
		...
		invoices.value.every((i) => selectedInvoices.value.includes(i.name))
)
const totalValue = computed(() =>
	invoices.value
		.filter((i) => selectedInvoices.value.includes(i.name))
...
function downloadJSON() {
	...
	const blob = new Blob([JSON.stringify(exportResult.value.export_data, null, 2)], {
