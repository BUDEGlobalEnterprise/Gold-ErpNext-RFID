	...
	<div class="min-h-screen bg-gray-50 dark:bg-[#0F1115] flex items-center justify-center p-4">
		<div class="max-w-md w-full">
			...
			<div v-if="loading" class="text-center py-20">
				<div
					class="w-16 h-16 mx-auto mb-6 rounded-full bg-[#D4AF37]/10 flex items-center justify-center"
				>
					<div
			...
			<!-- Success State -->
			<div
				v-else-if="paymentVerified"
				class="text-center"
				data-testid="payment-success-view"
			>
				<div
					class="bg-white dark:bg-[#1a1c23] rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 p-8 mb-6"
				>
					<div
					...
					<div
						class="bg-gray-50 dark:bg-[#15171e] rounded-xl p-5 border border-gray-100 dark:border-gray-800 space-y-3 text-left"
					>
						<div v-if="sessionData.repair_order" class="flex justify-between text-sm">
							...
							<span class="font-bold text-gray-900 dark:text-white font-mono">{{
								sessionData.repair_order
							}}</span>
						</div>
						<div class="flex justify-between text-sm">
							<span class="text-gray-500 dark:text-gray-400">Amount Paid</span>
							<span class="font-bold text-[#D4AF37] text-lg">{{
								formatCurrency(sessionData.amount_total)
							}}</span>
						</div>
						<div
							v-if="sessionData.customer_email"
				...
				<p class="text-xs text-gray-400 dark:text-gray-600 mt-4">
					You can safely close this window. A confirmation email will be sent to your
...
onMounted(async () => {
	...
	try {
		const result = await call(
			'zevar_core.api.repair_customer_portal.verify_checkout_session',
			{
