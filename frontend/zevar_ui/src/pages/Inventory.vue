<template>
	<AppLayout>
		<div class="flex flex-col h-full">
			<div class="flex items-center justify-between gap-4 mb-4 flex-shrink-0">
				...
				<div class="flex items-center gap-2">
					<button @click="showQuickAdd = true" class="hidden sm:flex items-center gap-1.5 px-3 py-2 bg-[#D4AF37] text-white rounded-lg text-xs font-bold hover:bg-[#C4A030] transition">
						<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
						Quick Add
					</button>
					<button @click="showPushToStores = true" class="hidden sm:flex items-center gap-1.5 px-3 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold hover:bg-blue-700 transition">
						<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>
						Push to Stores
					</button>
					<button @click="showTransferModal = true" class="hidden sm:flex items-center gap-1.5 px-3 py-2 border border-gray-300 rounded-lg text-xs font-bold hover:bg-gray-50 transition">
						New Transfer
					</button>
					<button @click="showConsignment = true" class="hidden sm:flex items-center gap-1.5 px-3 py-2 border border-gray-300 rounded-lg text-xs font-bold hover:bg-gray-50 transition">
						Consignment
					</button>

					<div class="md:hidden">
						<button @click="mobileMenuOpen = !mobileMenuOpen" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-warm-dark-700">
			...
			<div v-if="mobileMenuOpen" class="md:hidden grid grid-cols-2 gap-2 mb-4">
				<button @click="showQuickAdd = true; mobileMenuOpen = false" class="py-2 bg-[#D4AF37] text-white rounded-lg text-xs font-bold">Quick Add</button>
				<button @click="showPushToStores = true; mobileMenuOpen = false" class="py-2 bg-blue-600 text-white rounded-lg text-xs font-bold">Push to Stores</button>
				<button @click="showTransferModal = true; mobileMenuOpen = false" class="py-2 border rounded-lg text-xs font-bold">Transfer</button>
				<button @click="showConsignment = true; mobileMenuOpen = false" class="py-2 border rounded-lg text-xs font-bold">Consignment</button>
			</div>
			...
			<div class="flex-1 overflow-y-auto min-h-0 pr-2 custom-scrollbar">
					...
					<table class="w-full text-sm">
						<thead>
							<tr class="bg-gray-50 dark:bg-warm-dark-700 border-b border-gray-200 dark:border-warm-border/50">
								...
								<th class="text-center px-4 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Actions</th>
							</tr>
						</thead>
						<tbody>
							<tr v-for="item in filteredItems" :key="item.code" class="border-b border-gray-100 dark:border-warm-border/50 hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors cursor-pointer" @click="selectedItem = item">
								<td class="px-4 py-3">
									<div class="flex items-center gap-3">
										<div class="w-10 h-10 rounded-lg bg-gray-100 dark:bg-warm-dark-900 overflow-hidden shrink-0">
								...
								<td class="px-4 py-3 hidden sm:table-cell">
									<span class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400">{{ item.metal }}</span>
								</td>
								...
								<td class="px-4 py-3 text-right">
									<span class="text-xs font-bold" :class="item.stock <= 0 ? 'text-red-500' : item.stock < 5 ? 'text-amber-500' : 'text-green-600'">{{ item.stock }}</span>
								</td>
								<td class="px-4 py-3 text-right text-xs font-bold font-mono text-gray-900 dark:text-white">{{ formatCurrency(item.price) }}</td>
								<td class="px-4 py-3 text-center hidden lg:table-cell">
									<span class="text-[9px] font-bold px-2 py-1 rounded-full" :class="item.stock <= 0 ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' : item.stock < 5 ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400' : 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400'">
										{{ item.stock <= 0 ? 'OUT OF STOCK' : item.stock < 5 ? 'LOW STOCK' : 'IN STOCK' }}
									</span>
								</td>
								<td class="px-4 py-3 text-center hidden lg:table-cell">
									<div class="flex items-center justify-center gap-1">
										<button @click.stop="openLifecycle(item)" class="p-1 hover:bg-gray-100 dark:hover:bg-warm-dark-700 rounded" title="Lifecycle">
										...
										<button @click.stop="openReserve(item)" class="p-1 hover:bg-gray-100 dark:hover:bg-warm-dark-700 rounded" title="Reserve">
											<svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
										</button>
										<button @click.stop="openDamage(item)" class="p-1 hover:bg-gray-100 dark:hover:bg-warm-dark-700 rounded" title="Damage">
											<svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>
										</button>
									</div>
								</td>
							</tr>
				...
				<div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
					<div v-for="item in filteredItems" :key="item.code" class="premium-card !p-0 overflow-hidden group cursor-pointer" @click="selectedItem = item">
						<div class="aspect-square bg-gray-100 dark:bg-warm-dark-900 relative">
							...
							<div class="absolute top-2 right-2">
								<span class="text-[9px] font-bold px-1.5 py-0.5 rounded-full" :class="item.stock <= 0 ? 'bg-red-100 text-red-700' : item.stock < 5 ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'">{{ item.stock }} pcs</span>
							</div>
						</div>
						<div class="p-3">
							...
							<div class="flex items-center gap-1 mb-2">
								...
								<span class="text-[9px] font-bold px-1.5 py-0.5 rounded bg-gray-100 dark:bg-warm-dark-900 text-gray-600 dark:text-gray-400">{{ item.purity }}</span>
							</div>
		...
		<QuickAddItemModal v-if="showQuickAdd" @close="showQuickAdd = false" @created="onDataChanged" />
		<PushToStoresModal v-if="showPushToStores" :item-code="pushItemCode" @close="showPushToStores = false" @pushed="onDataChanged" />
		<StoreTransferModal v-if="showTransferModal" :order="{}" mode="dispatch" @close="showTransferModal = false" @completed="onDataChanged" />
		<ConsignmentModal v-if="showConsignment" mode="out" @close="showConsignment = false" @completed="onDataChanged" />
		<ReservePieceModal v-if="showReserve && reserveSerialNo" :serial-no="reserveSerialNo" @close="showReserve = false" @reserved="onDataChanged" />
		<DamageReportModal v-if="showDamage && damageSerialNo" :serial-no="damageSerialNo" @close="showDamage = false" @completed="onDataChanged" />
		<PieceLifecyclePanel v-if="showLifecycle && lifecycleSerialNo" :serial-no="lifecycleSerialNo" @close="showLifecycle = false" />
	</AppLayout>
</template>
