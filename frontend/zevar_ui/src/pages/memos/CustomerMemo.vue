<template>
	<AppLayout>
		<div class="flex flex-col h-full">
			...
			<div
				v-if="memos.length"
				class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-auto"
			>
				<div
					v-for="memo in memos"
					:key="memo.name"
					class="bg-white dark:bg-warm-dark-800 border border-gray-200 dark:border-warm-border rounded-xl p-4 hover:shadow-md transition-shadow"
				>
					<!-- Card Header -->
					<div class="flex items-center justify-between mb-3">
						<span class="text-sm font-semibold">{{ memo.customer_name }}</span>
						<span
							:class="dispositionClass(memo.final_disposition)"
					...
					<div class="flex gap-2 mb-3 overflow-x-auto pb-1">
						<div
							v-for="(item, idx) in memo.items?.slice(0, 4)"
							:key="idx"
					...
					<div class="flex items-center justify-between text-xs text-gray-500 mb-3">
						<span
							>Hold until:
							<strong :class="isOverdue(memo.due_date) ? 'text-red-500' : ''">{{
					...
					<!-- Signed Slip -->
					<div
						v-if="memo.customer_signed_return_slip"
						class="flex items-center gap-2 text-xs text-green-600 mb-3"
					>
					...
					<div class="flex gap-2 pt-2 border-t border-gray-100 dark:border-warm-border">
						<button
							@click="extendMemo(memo)"
							class="flex-1 text-xs py-1.5 border border-gray-200 rounded-lg hover:bg-gray-50 dark:hover:bg-warm-dark-700 transition-colors"
						>
							📅 Extend
						</button>
						<button
							@click="convertToSale(memo)"
							class="flex-1 text-xs py-1.5 border border-green-200 text-green-700 rounded-lg hover:bg-green-50 transition-colors"
						>
							💰 Sell
						</button>
						<button
							@click="recordReturn(memo)"
							class="flex-1 text-xs py-1.5 border border-blue-200 text-blue-700 rounded-lg hover:bg-blue-50 transition-colors"
						>
							↩ Return
						</button>
					</div>
...
function extendMemo(memo) {
	/* TODO */
}
function convertToSale(memo) {
	/* TODO */
}
function recordReturn(memo) {
	/* TODO */
}
