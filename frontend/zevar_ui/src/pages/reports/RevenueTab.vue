	...
	<div class="tab-content">
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
			<div class="premium-card">
				...
				<div v-else-if="!hourly.length" class="text-xs text-gray-400 text-center py-8">No hourly data</div>
			...
			<div class="premium-card">
				...
				<div v-if="loading" class="space-y-2">
					<div v-for="n in 4" :key="n" class="h-3 bg-gray-100 dark:bg-gray-800 rounded animate-pulse" />
				</div>
				<div v-else-if="!categories.length" class="text-xs text-gray-400 text-center py-8">No data</div>
				<div v-else class="space-y-2">
					<div v-for="c in categories" :key="c.category" class="flex items-center gap-3">
						<div class="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-3 overflow-hidden">
							<div class="h-full rounded-full bg-[#D4AF37] transition-all" :style="{ width: c.pct + '%' }" />
						</div>
						<span class="text-[10px] font-bold w-20 truncate text-right text-gray-700 dark:text-gray-300">{{ c.category }}</span>
						<span class="text-[10px] font-bold font-mono text-gray-900 dark:text-white">${{ fmt(c.total) }}</span>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="premium-card mt-4">
			...
			<div v-else-if="!salespeople.length" class="text-xs text-gray-400 text-center py-6">No data</div>
			<div v-else class="space-y-1">
				<div v-for="(sp, i) in salespeople" :key="sp.name" class="flex items-center gap-3 px-2 py-1.5 rounded hover:bg-gray-50 dark:hover:bg-[#1C1F26]">
					<span class="text-[10px] font-black text-gray-400 w-4">{{ i + 1 }}</span>
					<span class="text-xs font-bold flex-1 truncate text-gray-900 dark:text-white">{{ sp.name }}</span>
					<span class="text-xs font-black font-mono text-emerald-600 dark:text-emerald-400">${{ fmt(sp.total) }}</span>
				</div>
			</div>
		</div>
	</div>
