<template>
	<AppLayout>
		<div class="hub">
			<header class="hub__header">
				...
				<div class="flex items-center gap-2">
					...
					<button
						type="button"
						:disabled="store_loading"
						@click="refresh(true)"
						class="h-9 w-9 rounded-lg flex items-center justify-center border border-gray-200 dark:border-warm-border hover:border-[#D4AF37] text-gray-500 hover:text-[#D4AF37] disabled:opacity-50"
						aria-label="Refresh"
					>
						<span
							class="material-symbols-outlined"
							:class="{ 'animate-spin': store_loading }"
							>refresh</span
						>
					</button>
				</div>
			...
			<section v-if="aiBrief" class="hub__ai-brief glass-tier-1" aria-label="AI daily brief">
				<div class="flex items-center gap-2 mb-1">
					<span
						class="px-1.5 py-0.5 rounded text-[10px] font-black uppercase tracking-wider bg-[color:var(--color-gold)]/15 text-[color:var(--color-gold)]"
						>AI</span
					>
					<span class="text-[10px] font-mono text-gray-500">Today</span>
				</div>
				<p class="text-sm font-bold text-gray-900 dark:text-white">
					{{ aiBrief.headline || 'Today at a glance' }}
				</p>
			...
			<section v-if="alerts.length" class="hub__alerts" aria-label="Alerts">
				<div
					v-for="a in alerts"
					:key="a.key"
					class="glass-tier-1 hub__alert"
					@click="openCard(a.card)"
				>
					<span
						class="material-symbols-outlined hub__alert-icon"
						:style="{ color: a.color }"
						>{{ a.icon }}</span
					>
		...
		<HubDrawer
			...
			:title="drawer.current.value?.title || 'Detail'"
			@close="drawer.close()"
		>
			<component
				:is="currentDrawerComponent"
				:payload="drawer.current.value?.payload"
				:kind="drawer.current.value?.kind"
			/>
		</HubDrawer>
	</AppLayout>
</template>
...
import {
	useFilteredTabs,
	TAB_COMPONENT_MAP,
	DRAWER_COMPONENT_MAP,
} from '@/composables/analytics/useHubTabs'
...
const asOfLabel = computed(() => {
	...
	try {
		return new Date(asOf.value).toLocaleString(undefined, {
			dateStyle: 'medium',
			timeStyle: 'short',
		})
	} catch {
...
const currentTabComponent = computed(
	() => TAB_COMPONENT_MAP[activeTab.value] || TAB_COMPONENT_MAP.revenue
)
const currentDrawerComponent = computed(
	() => DRAWER_COMPONENT_MAP[drawer.current.value?.kind] || null
)

function openCard(key) {
	hub.openCardDrawer(key)
}
