		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
			<div class="premium-card">
				...
				<div v-else class="space-y-2">
					...
					<div class="flex items-center gap-3">
						<span class="text-xs w-24 text-gray-600 dark:text-gray-300"
							>Returning</span
						>
						<div
							class="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-3 overflow-hidden"
			...
			<div class="premium-card">
				...
				<div v-else-if="!top.length" class="text-xs text-gray-400 text-center py-6">
					No data
				</div>
...
const pctNew = computed(() =>
	total.value ? (Number(split.value.new || 0) / total.value) * 100 : 0
)
const pctReturning = computed(() =>
	total.value ? (Number(split.value.returning || 0) / total.value) * 100 : 0
)
